...
public class L1BookMark {
	...
	public static void deleteBookmark(L1PcInstance player, String s) {
		...
		if (book != null) {
			...
			try {
				...
				_log.error("Bookmark to remove the error occurred.", e);
			} finally {
				SQLUtil.close(pstm);
				SQLUtil.close(con);
			}
	...
	public static void setQuickList(L1PcInstance pc, ArrayList<Integer> quicklistIds) {
		...
		try {
			...
			if (quicklistIds.size() > 0) {
				...
				pstm = con
						.prepareStatement("UPDATE character_teleport SET quicklist = true WHERE char_id = ? AND id IN ("
	...
	public static void addBookmark(L1PcInstance pc, String s) {
		...
		if (pc.getBookMark(s) == null) {
			...
			pc.sendPackets(
					new S_Bookmarks(s, bookmark.getMapId(), bookmark.getId(), bookmark.getLocX(), bookmark.getLocY()));
		} else {
