...
public class C_Chat extends ClientBasePacket {
	...
	private static void sendChatPacket(final L1PcInstance sender, final Iterable<L1PcInstance> receivers,
			final String text, final int type, final int opcode) {
	...
	private static Iterable<L1PcInstance> filterForIgnored(final L1PcInstance sender,
		...
		List<L1PcInstance> filtered = new ArrayList<>();
	...
	public C_Chat(byte abyte0[], Client clientThread) {
		...
		if (pc.hasSkillEffect(1005) || (pc.getMapId() == 99 && !pc.isGm() && chatType != 0)) { // jailed users can't
																								// chat!
		...
		switch (chatType) {
			...
			if (chatText.startsWith("!")) {
				...
				normalChat(pc, chatText, 2, SHOUT_RANGE, Opcodes.S_OPCODE_NORMALCHAT);
			...
			normalChat(pc, chatText, chatType, Config.PC_RECOGNIZE_RANGE, Opcodes.S_OPCODE_NORMALCHAT);
			break;
		case 2:
			if (pc.isGhost() && !pc.isGm())
				return;
			...
			normalChat(pc, chatText, chatType, SHOUT_RANGE, Opcodes.S_OPCODE_NORMALCHAT);
			break;
		case 3:
			worldChat(pc, chatText, chatType);
			break;
		case 4:
			clanChat(pc, chatText, chatType, L1Clan.CLAN_RANK_PUBLIC, L1Clan.CLAN_RANK_PROBATION);
			break;
		case 11:
			if (!pc.isInParty())
				return;

			sendChatPacket(pc, Arrays.asList(pc.getParty().getMembers()), chatText, chatType,
		...
		case 12:
			...
			break;
		case 13:
			clanChat(pc, chatText, chatType, L1Clan.CLAN_RANK_GUARDIAN, L1Clan.CLAN_RANK_GUARDIAN);
			break;
		case 14:
			if (!pc.isInChatParty())
				return;

			sendChatPacket(pc, Arrays.asList(pc.getChatParty().getMembers()), chatText, chatType,
	...
	private void normalChat(final L1PcInstance sender, final String text, final int type, final int range,
		...
		List<L1PcInstance> receivers = L1World.getInstance().getVisiblePlayer(sender, range);
		receivers.add(sender);

		sendChatPacket(sender, receivers, text, type, Opcodes.S_OPCODE_NORMALCHAT);

		echo(sender, text, type, range);
	}

	private void echo(final L1PcInstance sender, final String text, final int type, final int range) {
		for (L1Object obj : sender.getKnownObjects()) {
			...
			if (!mob.getNpcTemplate().is_doppel() || !mob.getName().equals(sender.getName()) || mob.getCurrentHp() <= 0)
				continue;

			S_NpcChatPacket packet = new S_NpcChatPacket(mob, text, type);
			for (L1PcInstance player : L1World.getInstance().getVisiblePlayer(mob, range))
				player.sendPackets(packet);
		}
	}

	private void worldChat(final L1PcInstance sender, final String text, final int type) {
		if (!canWorldChat(sender))
			return;

		List<L1PcInstance> receivers = new ArrayList<>();
		for (L1PcInstance player : L1World.getInstance().getAllPlayers())
			if ((type == 12 && player.showTradeChat()) || (type == 3 && player.showWorldChat()))
				receivers.add(player);

		sendChatPacket(sender, receivers, text, type, Opcodes.S_OPCODE_GLOBALCHAT);
	}

	private void clanChat(final L1PcInstance sender, final String text, final int type, final int minSenderRank,
			final int minReceiverRank) {
	...
	private boolean canWorldChat(final L1PcInstance sender) {
		...
		if (sender.getLevel() <= Config.GLOBAL_CHAT_LEVEL) {
			sender.sendPackets(new S_ServerMessage(195, String.valueOf(Config.GLOBAL_CHAT_LEVEL)));
			return false;
		}
		...
		/*
		 * TRICIDTODO: Make this a configurable option. if (sender.get_food() <= 2) {
		 * pc.sendPackets(new S_ServerMessage(462)); return false; } else {
