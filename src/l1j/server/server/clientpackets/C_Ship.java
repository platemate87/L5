...
public class C_Ship extends ClientBasePacket {
	...
	public C_Ship(byte abyte0[], Client client) {
		...
		// if it is null, they're not on a boat, and if the coord is not in the
		// acceptable range, crap out
		// since the coord seems to be a few places around the dock, and I didn't want
		// to bother finding them all
		// I am just doing a generic check for them being within <MAX_DISTANCE> cells of
		// the place the client is trying to send
		if (getBack == null || !validXCoord(getBack, locX) || !validYCoord(getBack, locY)) {
			_log.warn(String.format("%s attempted to exploit C_SHIP to move to X: %d, Y: %d, MapId: %d.", pc.getName(),
					locX, locY, shipMapId));
			return;
		}
	...
	// the getback loc is just a rough idea, so we check to see if they are within
	// 20 cells of this
