...
public class C_PickUpItem extends ClientBasePacket {
	...
	public C_PickUpItem(byte decrypt[], Client client) throws Exception {
		...
		if (pc.isDead() || pc.isGhost() || pc.isInvisble() || pc.isInvisDelay()) {
			return;
		}
		...
		if (object != null && !pc.isDead()) {
			...
			if ((!item.isStackable() && pickupCount != 1) || item.getCount() <= 0 || pickupCount <= 0
					|| pickupCount > 2000000000 || pickupCount > item.getCount()) {
				if (Config.AUTO_BAN) {
			...
			if (objectId != item.getId()) {
				_log.warn(pc.getName() + " had item " + Integer.toString(objectId) + " not match.");
			}
			...
			if (pc.getInventory().checkAddItem(item, pickupCount) == L1Inventory.OK) {
				if (item.getX() != 0 && item.getY() != 0) {
					...
					lpui.storeLogPickUpItem(pc, item, before_inven, after_inven, brfore_ground, after_ground,
							pickupCount);
