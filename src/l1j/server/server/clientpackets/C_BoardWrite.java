...
public class C_BoardWrite extends ClientBasePacket {
	...
	public C_BoardWrite(byte decrypt[], Client client) {
		...
		if (tg != null) {
			...
			if (tg instanceof L1NpcInstance && ((L1NpcInstance) tg).getSpawn() == NpcSpawnTable.getBugBoard()) {
				Connection con = null;
				...
				try {
					...
					pstm = con.prepareStatement(
							"INSERT INTO bugs (bugtext, charname, mapID, mapX, mapY, resolved, title, submitted) VALUES (?, ?, ?, ?, ?, 0, ?, NOW());");
					...
					_log.warn(String.format("%s submitted a bug report, but an error occurred! Bug Text: %s",
							pc.getName(), content));
				} finally {
					SQLUtil.close(pstm);
					SQLUtil.close(con);
