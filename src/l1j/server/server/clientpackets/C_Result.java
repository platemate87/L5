...
public class C_Result extends ClientBasePacket {
	...
	public C_Result(byte abyte0[], Client client) throws Exception {
		...
		if (resultType == 0 && size != 0 && npcImpl.equals("") && npcObjectId == pc.getId() && pc.isGm()) {
			npcImpl = "L1Merchant";
			npcId = pc.getId();
		}
		...
		if (resultType == 0 && size != 0 && npcImpl.equalsIgnoreCase("L1Merchant")) {
			...
			// if the shop is one created by the GM action, then remove it now that we've
			// used it
			...
			for (int i = 0; i < size; i++) {
				...
				if (count > item.getCount()) {
					_log.error(pc.getName() + " tried to store " + count + " of " + objectId + " but server has "
							+ item.getCount() + ".");
					ban(pc);
					return;
				}
			...
			for (int i = 0; i < size; i++) {
				...
				int item_count_after = 0;
				if (count > item.getCount()) {
					_log.error(pc.getName() + " tried to retrieve " + count + " of " + objectId + " but server has "
							+ item.getCount() + ".");
					ban(pc);
					return;
				}
			...
			if (pc.getClanid() != 0) {
				for (int i = 0; i < size; i++) {
					...
					// int item_count_after = 0;
					if (count > item.getCount()) {
						_log.error(pc.getName() + " tried to (clan) store " + count + " of " + objectId
			...
			if (clan != null) {
				for (int i = 0; i < size; i++) {
					...
					// int item_count_after = 0;
					if (count > item.getCount()) {
						_log.error(pc.getName() + " tried to (clan) retrieve " + count + " of " + objectId
								+ " but server has " + item.getCount() + ".");
						ban(pc);
						return;
					}
			...
			for (int i = 0; i < size; i++) {
				...
				int item_count_after = 0;
				if (count > item.getCount()) {
					_log.error(pc.getName() + " tried to (elf) store " + count + " of " + objectId + " but server has "
							+ item.getCount() + ".");
					ban(pc);
					return;
				}
			...
			for (int i = 0; i < size; i++) {
				...
				int item_count_after = 0;
				if (count > item.getCount()) {
					_log.error(pc.getName() + " tried to (elf) retrieve " + count + " of " + objectId
							+ " but server has " + item.getCount() + ".");
					ban(pc);
					return;
				}
					...
					if (pc.getInventory().checkAddItem(item, count) == L1Inventory.OK) {
						...
						if (pc.getInventory().checkItem(L1ItemId.ADENA, price)) {
							...
							if (pc != null && targetPc != null && adena != null) {
								...
								long itemTransactionId = lpsb.storeLogPrivateShopBuy(transactionId, pc, targetPc, item,
			...
			for (int i = 0; i < size; i++) {
				...
				if (item.getItemId() != buyItemId && item.getEnchantLevel() == buyEnchantLevel) {
					...
					L1World.getInstance()
							.broadcastServerMessage("Player " + pc.getName() + " Attempted A Private Shop exploit!");
				...
				if (targetPc.getInventory().checkAddItem(item, count) == L1Inventory.OK) {
					...
					if (targetPc.getInventory().checkItem(L1ItemId.ADENA, count * buyPrice)) {
						...
						if (pc != null && targetPc != null && adena != null) {
							...
							long itemTransactionId = lpss.storeLogPrivateShopSell(transactionId, pc, targetPc, item,
									item_count_before, count);

							if (pc.getInventory().tradeItem(item, count, targetPc.getInventory()) == null) {
								targetPc.setTradingInPrivateShop(false);
								return;
							}
							...
							int adena_before = adena.getCount();
							// log the start of the trade of the adena
							long adenaTransactionId = lpss.storeLogPrivateShopSell(transactionId, targetPc, pc, adena,
									adena_before, count);
