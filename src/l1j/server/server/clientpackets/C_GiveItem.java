...
public class C_GiveItem extends ClientBasePacket {
	...
	public C_GiveItem(byte decrypt[], Client client) {
		...
		if ((!item.isStackable() && count != 1) || item.getCount() <= 0 || count <= 0 || count > 2000000000
				|| count > item.getCount()) {
			if (Config.AUTO_BAN) {
		...
		if (itemId != item.getId()) {
			_log.warn(pc.getName() + " had item " + Integer.toString(itemId) + " not match.");
		}
		if (item.isEquipped()) {
		...
		try {
			...
			giveItemLog.storeLogGiveItem(pc, target, item, before_inv, after_inv, before_target_inv, after_target_inv,
					count);
		} catch (Exception ex) {
			_log.warn(String.format("%s Dropped item %s (%d). But it failed to log!", pc.getName(), item.getName(),
					item.getCount()));
		}
		...
		if (item.getItemId() == 40070 && petType.canEvolve()) {
			if (petType.getBaseNpcId() == 45686 || petType.getBaseNpcId() == 45687 || petType.getBaseNpcId() == 45688
					|| petType.getBaseNpcId() == 45689 || petType.getBaseNpcId() == 45690
					|| petType.getBaseNpcId() == 45691 || petType.getBaseNpcId() == 45692
					|| petType.getBaseNpcId() == 45693 || petType.getBaseNpcId() == 45694
					|| petType.getBaseNpcId() == 45695 || petType.getBaseNpcId() == 45696
					|| petType.getBaseNpcId() == 45697 || petType.getBaseNpcId() == 45710
		...
		if (item.getItemId() == 41310 && petType.canEvolve()) { // added for
																// Golden Dragon
			if (petType.getBaseNpcId() == 45034 || petType.getBaseNpcId() == 45039 || petType.getBaseNpcId() == 45040
					|| petType.getBaseNpcId() == 45042 || petType.getBaseNpcId() == 45043
					|| petType.getBaseNpcId() == 45044 || petType.getBaseNpcId() == 45046
					|| petType.getBaseNpcId() == 45047 || petType.getBaseNpcId() == 45048
					|| petType.getBaseNpcId() == 45049 || petType.getBaseNpcId() == 45053
					|| petType.getBaseNpcId() == 45054 || petType.getBaseNpcId() == 45313
					|| petType.getBaseNpcId() == 45712) {
	...
	private final static String receivableImpls[] = new String[] { "L1Npc", "L1Monster", "L1Guardian", "L1Teleporter",
			"L1Guard" };
	...
	private boolean isTamePet(L1NpcInstance npc) {
		...
		if (npcId == 45313) {
			if (npc.getMaxHp() / 3 > npc.getCurrentHp() && ThreadLocalRandom.current().nextInt(16) == 15) {
				isSuccess = true;
			}
		} else {
