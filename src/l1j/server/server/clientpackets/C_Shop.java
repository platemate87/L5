		...
		if (type == 0) {
			...
			for (int i = 0; i < sellTotalCount; i++) {
				...
				if (sellObjectId != checkItem.getId() || (!checkItem.isStackable() && sellCount != 1)
						|| checkItem.getCount() <= 0 || sellCount <= 0 || sellCount > 2000000000
					...
					_log.info(pc.getName() + " Attempted Dupe Exploit (C_Shop).");
					L1World.getInstance()
							.broadcastServerMessage("Player " + pc.getName() + " Attempted A Dupe exploit!");
					pc.sendPackets(new S_Disconnect());
					return;
				}
			...
			for (int i = 0; i < buyTotalCount; i++) {
				...
				if (buyObjectId != checkItem.getId() || buyCount <= 0 || buyCount > 2000000000 || buyPrice <= 0
						|| buyPrice > 2000000000) {
					if (Config.AUTO_BAN) {
			...
			if (buyList.size() > 8 || sellList.size() > 8) {
				...
				_log.warn(String.format(
						"Character '%s' attempted to add more than 8 items to sell/buy shop. "
