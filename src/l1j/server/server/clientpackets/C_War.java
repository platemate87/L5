	...
	public C_War(byte abyte0[], Client client) throws Exception {
		...
		if (enemyClan.getCastleId() != 0) {
			...
			for (L1PcInstance member : clanMembersOnline) {
				if (member.getLevel() >= Config.CASTLE_WAR_MIN_PRINCE_LEVEL && member.getId() != player.getId()) {
			...
			if (player.getLevel() < Config.CASTLE_WAR_MIN_PRINCE_LEVEL
				...
				if (player.getLevel() < Config.CASTLE_WAR_MIN_PRINCE_LEVEL) {
					player.sendPackets(new S_SystemMessage(
							String.format("\\fY  You are not above level %d.", Config.CASTLE_WAR_MIN_PRINCE_LEVEL)));
				}

				if (onlineMeetingLevelReq < Config.CASTLE_WAR_MIN_MEMBERS_ONLINE) {
					player.sendPackets(new S_SystemMessage(
							String.format("\\fY  You must have %d " + "members online above level %d.",
									Config.CASTLE_WAR_MIN_MEMBERS_ONLINE, Config.CASTLE_WAR_MIN_MEMBERS_LEVEL)));
				}
