...
public class C_CharReset extends ClientBasePacket {
	 ...
	 */
	private void checkProvidedStats(final L1PcInstance pc, int str, int intel, int wis, int dex, int con, int cha,
			Client client) {
		Map<L1Attribute, Integer> fixedStats = pc.getClassFeature().getFixedStats();
		if (str < fixedStats.get(L1Attribute.Str) || intel < fixedStats.get(L1Attribute.Int)
				|| wis < fixedStats.get(L1Attribute.Wis) || dex < fixedStats.get(L1Attribute.Dex)
				|| con < fixedStats.get(L1Attribute.Con) || cha < fixedStats.get(L1Attribute.Cha))
		...
		if (str < pc.getBaseStr() || intel < pc.getBaseInt() || wis < pc.getBaseWis() || dex < pc.getBaseDex()
				|| con < pc.getBaseCon() || cha < pc.getBaseCha()) {
			emergencyCleanup(pc, String.format("Candle: %s tried to redistribute stats after level ups!", pc.getName()),
					"Candle: issue with stats, contact a GM for help.", client);
		}
	}

	private void checkProvidedStartingStats(final L1PcInstance pc, int str, int intel, int wis, int dex, int con,
			int cha, Client client) {
		...
		if ((str < originalStr || dex < originalDex || con < originalCon || wis < originalWis || cha < originalCha
				|| intel < originalInt)
				|| (str > originalStr + originalAmount || dex > originalDex + originalAmount
						...
						|| cha > originalCha + originalAmount || intel > originalInt + originalAmount)) {
			isStatusError = true;
		}
		...
		if (str > startingMaxSTR || dex > startingMaxDex || con > startingMaxCon || wis > startingMaxWis
				|| cha > startingMaxCha || intel > startingMaxInt) {
			isStatusError = true;
		}
	...
	public C_CharReset(byte abyte0[], Client client) {
		...
		// TODO -- not sure this check is actually required, but it crashes on 3.63
		// because CharReset is used for more than just candling
		/*
		 * if(pc.getMapId() != 997) { emergencyCleanup(pc, String.
		 * format("%s attempted to candle on an incorrect map (may have sent the packet manually)! Mapid: %d"
		 * , pc.getName(), pc.getMapId()), "Error candling. Contact a GM.", client);
		 * _log.warn(String.
		 * format("%s has sent a candle packet while on a non-candle map! MapId: %d",
		...
		if (stage == 0x01) { // 0x01:LN^[
			...
			if (type2 == 0x00) { // 0x00:Lv1UP
				...
				int fullStats = pc.getBaseStr() + pc.getBaseInt() + pc.getBaseWis() + pc.getBaseDex() + pc.getBaseCon()
						+ pc.getBaseCha();
				int originalStats = pc.getOriginalStr() + pc.getOriginalInt() + pc.getOriginalWis()
						+ pc.getOriginalDex() + pc.getOriginalCon() + pc.getOriginalCha();
				...
				if (increasedStats + 1 > possiblePoints) {
					emergencyCleanup(pc,
							String.format(
									"%s has attemped to exploit their stats! Increased Stats: %d, Possible Stats: %d",
									pc.getName(), increasedStats, possiblePoints),
	...
	private synchronized void emergencyCleanup(final L1PcInstance pc, final String logEntry, final String message,
			Client client) {
		_log.error(logEntry);
	...
	private synchronized void saveNewCharStatus(L1PcInstance pc, Client client) {
		if (pc.getTempMaxLevel() != pc.getLevel()) {
			emergencyCleanup(pc, "Candle: " + pc.getName() + "'s level " + "doesn't match!",
	...
	private void initCharStatus(L1PcInstance pc, int hp, int mp, int str, int intel, int wis, int dex, int con,
			int cha) {
