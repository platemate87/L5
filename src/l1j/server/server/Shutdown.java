...
public class Shutdown extends Thread {
	...
	/**
	 * get the shutdown-hook instance the shutdown-hook instance is created by the
	 * first call of this function, but it has to be registrered externaly.
	 *
	 ...
	 *
	 * if this is not the thread of getInstance, then this is a countdown thread. we
	 * start the countdown, and when we finished it, and it was not aborted, we tell
	 * the shutdown-hook why we call exit, and then call exit
	 *
	...
	public void startShutdown(L1PcInstance activeChar, int seconds, boolean restart) {
		...
		_log.warn("GM: " + activeChar.getId() + " issued shutdown command. " + _modeText[shutdownMode] + " in "
				+ seconds + " seconds!");
	...
	/**
	 * this counts the countdown and reports it to all players countdown is aborted
	 * if mode changes to ABORT
	 */
	...
	public void startTelnetShutdown(String IP, int seconds, boolean restart) {
		Announcements _an = Announcements.getInstance();
		_log.warn(
				"IP: " + IP + " issued shutdown command. " + _modeText[shutdownMode] + " in " + seconds + " seconds!");
		_an.announceToAll("Server is " + _modeText[shutdownMode] + " in " + seconds + " seconds!");
