...
public class L1Attack {
	...
	private static final int[] PREVENT_DAMAGE = { ABSOLUTE_BARRIER, ICE_LANCE, FREEZING_BLIZZARD, FREEZING_BREATH,
			EARTH_BIND };
	...
	public L1Attack(L1Character attacker, L1Character target, int skillId) {
		if (attacker instanceof L1PcInstance) {
			...
			if (weapon != null) {
				...
				_weaponEnchant = weapon.getEnchantLevel() - (isRanged ? 0 : weapon.get_durability());
	...
	private int getBasePcHit() {
		...
		hitRate += isRanged ? _pc.getBowHitup() + _pc.getOriginalBowHitup() + _pc.getBowHitModifierByArmor()
				: _pc.getHitup() + _pc.getOriginalHitup() + _pc.getHitModifierByArmor();
	...
	/*
	 * C's hit rate = (PC-class Lv + + STR correction correction correction + + DEX
	 * + DAI the number of weapons correction / 2 + magic correction) ~ 0.68-10 The
	 * figures are calculated in their biggest hit (95%) gives the other side of the
	 * PC can take the other side of the PC's AC AC is better at self-hit rate of 1
	 * per pull out The minimum rate hit 5 percent hit rate of up to 95%
	 */
	...
	/**
	 * Whether or not the attacker (still) needs a buff to affect the target. Only
	 * arises in pve situations, e.g. attacking Chaos. See notes in L1Magic.
	 */
	...
	private boolean isMissingSkillEffect(L1PetInstance pet) {
		...
		if ((_calcType == PC_NPC || isPet) && _targetNpc != null) {
	...
	private static int getNpcHitRate(final L1NpcInstance attacker, final L1Character target) {
		...
		int defenderDice = targetAc >= 0 ? 10 - targetAc : 10 + _random.nextInt(defenderValue) + 1;
	...
	private double calcPcBaseDamage(boolean small, boolean materialBonus, boolean hardSkinned, boolean diceDagger) {
		int weaponMaxDamage = small ? _weaponSmall : _weaponLarge;
		int weaponDamage = 0;
		if (_weaponType == WeaponType.Claw && ThreadLocalRandom.current().nextInt(100) + 1 <= _weaponDoubleDmgChance) {
		...
		double damage = weaponTotalDamage + _statusDamage
				+ (isRanged ? _pc.getBowDmgup() + _pc.getOriginalBowDmgup() : _pc.getDmgup() + _pc.getOriginalDmgup());

		if (isBow) {
			if (_arrow != null) {
				int arrowDamage = small ? _arrow.getItem().getDmgSmall() : _arrow.getItem().getDmgLarge();
				if (arrowDamage == 0)
					arrowDamage = 1;
				if (hardSkinned)
					arrowDamage /= 2;
			...
			int knifeDamage = small ? _sting.getItem().getDmgSmall() : _sting.getItem().getDmgLarge();
			if (knifeDamage == 0)
				knifeDamage = 1;
		...
		if ((_weaponId == DICE_DAGGER || _weaponId == C_DICE_DAGGER) && diceDagger) {
			...
			damage += L1WeaponSkill.getWeaponSkillDamage(_pc, _target, _weaponId);
		}
		...
		damage += isRanged ? _pc.getBowDmgModifierByArmor() : _pc.getDmgModifierByArmor();
	...
	public int calcPcPcDamage() {
		...
		if (_targetPc.hasSkillEffect(COOKING_1_0_S) || _targetPc.hasSkillEffect(COOKING_1_1_S)
				|| _targetPc.hasSkillEffect(COOKING_1_2_S) || _targetPc.hasSkillEffect(COOKING_1_3_S)
				|| _targetPc.hasSkillEffect(COOKING_1_4_S) || _targetPc.hasSkillEffect(COOKING_1_5_S)
				|| _targetPc.hasSkillEffect(COOKING_1_6_S) || _targetPc.hasSkillEffect(COOKING_2_0_S)
				|| _targetPc.hasSkillEffect(COOKING_2_1_S) || _targetPc.hasSkillEffect(COOKING_2_2_S)
				|| _targetPc.hasSkillEffect(COOKING_2_3_S) || _targetPc.hasSkillEffect(COOKING_2_4_S)
				|| _targetPc.hasSkillEffect(COOKING_2_5_S) || _targetPc.hasSkillEffect(COOKING_2_6_S)
				|| _targetPc.hasSkillEffect(COOKING_3_0_S) || _targetPc.hasSkillEffect(COOKING_3_1_S)
				|| _targetPc.hasSkillEffect(COOKING_3_2_S) || _targetPc.hasSkillEffect(COOKING_3_3_S)
				|| _targetPc.hasSkillEffect(COOKING_3_4_S) || _targetPc.hasSkillEffect(COOKING_3_5_S)
				|| _targetPc.hasSkillEffect(COOKING_3_6_S)) {
			dmg -= 5;
		}
		if (_targetPc.hasSkillEffect(COOKING_1_7_S) || _targetPc.hasSkillEffect(COOKING_2_7_S)
	...
	private int calcPcNpcDamage() {
		boolean small = _targetNpc.getNpcTemplate().get_size().equalsIgnoreCase("small");
	...
	private int calcMaterialBlessDmg() {
		...
		if (_pc.getWeapon() != null && !isRanged && weapon.getHolyDmgByMagic() != 0 && (undead == 1 || undead == 3)) {
	...
	private int calcAttrEnchantDmg() {
		...
		if (_calcType == PC_PC) {
			...
			if ((_weaponAttrEnchantKind & _targetNpc.getNpcTemplate().get_weakAttr()) == _weaponAttrEnchantKind) {
				resist = -50;
			}
		}
	...
	public void calcStaffOfMana() {
		...
		/*
		 * Edited to work the same as SOM above. Left in its own if block so we can
		 * easily adjust it later if we decide this is OP
		 */
	...
	public void addChaserAttack() {
		if (_weaponId != THEBES_BOW && _weaponId != THEBES_STAFF && _weaponId != THEBES_SWORD
				&& _weaponId != THEBES_EDORYU)
			return;
...
//		}
	...
	private void actionPc() {
		...
		if (isBow) {
			if (_arrow != null) {
				_pc.sendAndBroadcast(new S_UseArrowSkill(_pc, _targetId, 66, _targetX, _targetY, _isHit));
				_pc.getInventory().removeItem(_arrow, 1);
			} else if (_weaponId == SAYHAS_BOW) {
				_pc.sendAndBroadcast(new S_UseArrowSkill(_pc, _targetId, 2349, _targetX, _targetY, _isHit));
			}
		} else if (isGauntlet && _sting != null) {
			_pc.sendAndBroadcast(new S_UseArrowSkill(_pc, _targetId, 2989, _targetX, _targetY, _isHit));
			_pc.getInventory().removeItem(_sting, 1);
		} else {
			if (_isHit) {
				_pc.sendAndBroadcast(new S_AttackPacket(_pc, _targetId, ActionCodes.ACTION_Attack));
			} else {
				if (_targetId > 0) {
					_pc.sendAndBroadcast(new S_AttackMissPacket(_pc, _targetId));
				} else {
					_pc.sendAndBroadcast(new S_AttackPacket(_pc, 0, ActionCodes.ACTION_Attack));
				}
			}
		}
		if (_isHit)
		...
		if (isLongRange && bowActId > 0) {
			...
			if (_isHit) {
				if (getGfxId() > 0) {
					...
					_target.broadcastPacketExceptTargetSight(new S_DoActionGFX(_targetId, ActionCodes.ACTION_Damage),
							_npc);
				} else {
	...
	private void damageNpcWeaponDurability() {
		 ...
		 */
		if (_calcType != PC_NPC || !_targetNpc.getNpcTemplate().is_hard() || _weaponType == WeaponType.Fist
				|| weapon.getItem().get_canbedmg() == 0 || _pc.hasSkillEffect(SOUL_OF_FLAME)) {
			return;
		}
		// normal weapons cursed weapons
		if ((_weaponBless == 1 || _weaponBless == 2) && ((ThreadLocalRandom.current().nextInt(100) + 1) < chance)) {
			// your injury is 0%.
	...
	private void damagePcWeaponDurability() {
		...
		if (_calcType != PC_PC || _weaponType == WeaponType.Fist || isRanged
				|| !_targetPc.hasSkillEffect(BOUNCE_ATTACK) || _pc.hasSkillEffect(SOUL_OF_FLAME)) {
			return;
		}
