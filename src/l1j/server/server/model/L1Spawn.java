...
public class L1Spawn {
	 ...
	 * SpawnTask To start.
	 *
	 * @param spawnNumber L1Spawn Which is administered by the numbers. Home to
	 *                    specify what point does not exist and it's good.
	 */
	...
	public void init() {
		...
		if (getSpawnHome() == 1 && getSpawnHomeCount() <= getAmount() && getSpawnHomeDelay() >= getMinRespawnDelay()
				&& isAreaSpawn()) {
	...
	/**
	 * If you have a point home, spawnNumber based spawn. If not, spawnNumber not
	 * used.
	 */
				...
				default:
					if (isAreaSpawn()) { // Coordinates of the range specified
						...
						if (!_initSpawn && _spawnHomePoint) { // Home to the
							...
							L1Location loc = new L1Location(point, getMapId()).randomLocation(getSpawnHomeRange(),
									false);
							newlocx = loc.getX();
							newlocy = loc.getY();
						} else {
