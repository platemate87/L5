	...
	private Dungeon() {
		...
		try {
			...
			while (rs.next()) {
				...
				if ((srcX == 33423 || srcX == 33424 || srcX == 33425 || srcX == 33426) && srcY == 33502 && srcMapId == 4 // nClD->FIsD
						|| (srcX == 32733 || srcX == 32734 || srcX == 32735 || srcX == 32736) && srcY == 32794
								&& srcMapId == 83) { // FIsD->nClD
				...
				if (_dungeonMap.containsKey(key)) {
					_log.warn("Dungeon exists for the same key data. key=" + key);
				}
		...
		if (_dungeonMap.containsKey(key)) {
			...
			if (dungeonType == DungeonType.NONE) {
				...
				if (nowtime >= 15 * 360 && nowtime < 25 * 360 // 1.30~2.30
						...
						|| nowtime >= 135 * 360 && nowtime < 145 * 360 || nowtime >= 165 * 360 && nowtime < 175 * 360
