...
public class L1DwarfForElfInventory extends L1Inventory {
	...
	private static Logger _log = LoggerFactory.getLogger(L1DwarfForElfInventory.class.getName());
	...
	public void loadItems() {
		...
		try {
			...
			pstm = con.prepareStatement(
					"SELECT * FROM character_elf_warehouse WHERE account_name = ? ORDER BY item_name");
	...
	public void insertItem(L1ItemInstance item) {
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement(
					"INSERT INTO character_elf_warehouse SET id = ?, account_name = ?, item_id = ?, item_name = ?, count = ?, is_equipped=0, enchantlvl = ?, is_id = ?, durability = ?, charge_count = ?, remaining_time = ?, last_used = ?, bless = ?, attr_enchant_kind = ?, attr_enchant_level = ?, m_def = ?, add_hp = ?, add_mp = ?, add_hpr = ?, add_mpr = ?, add_sp = ?, defense_water = ?, defense_wind = ?, defense_fire = ?, defense_earth = ?");
