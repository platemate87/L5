...
public class L1Cube implements Runnable {
	...
	public void run() {
		try {
			...
			if (_cha.isDead() || !_cha.hasSkillEffect(_skillId)) {
				stop();
				return;
			}
			_timeCounter++;
	...
	public void giveEffect() {
		if (_skillId == STATUS_CUBE_IGNITION_TO_ENEMY) {
			if ((_timeCounter % 4 != 0) || _cha.hasSkillEffect(STATUS_FREEZE) || _cha.hasSkillEffect(ABSOLUTE_BARRIER) || _cha.hasSkillEffect(ICE_LANCE)) {
				return;
			}
