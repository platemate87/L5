...
public class L1DwarfForClanInventory extends L1Inventory {
	...
	private static Logger _log = LoggerFactory.getLogger(L1DwarfForClanInventory.class.getName());
	...
	public synchronized void loadItems() {
		...
		try {
			...
			pstm = con.prepareStatement("SELECT * FROM clan_warehouse WHERE clan_name = ? ORDER BY item_name");
	...
	public synchronized void insertItem(L1ItemInstance item) {
		...
		try {
			...
			pstm = con.prepareStatement(
					"INSERT INTO clan_warehouse SET id = ?, clan_name = ?, item_id = ?, item_name = ?, count = ?, is_equipped=0, enchantlvl = ?, is_id= ?, durability = ?, charge_count = ?, remaining_time = ?, last_used = ?, bless = ?, attr_enchant_kind = ?, attr_enchant_level = ?, m_def = ?, add_hp = ?, add_mp = ?, add_hpr = ?, add_mpr = ?, add_sp = ?, defense_water = ?, defense_wind = ?, defense_fire = ?, defense_earth = ?");
