...
//		}
	...
	private boolean isNormalSkillUsable() {
		if (_user instanceof L1PcInstance) {
			...
			if (pc.isParalyzed() || ((pc.isInvisble() || pc.isInvisDelay()) && !isInvisUsableSkill(_skillId))) {
				return false;
			}
	...
	private boolean isTarget(L1Character cha) {
		...
		try {
			...
			if ((_skill.getType() == L1Skill.TYPE_ATTACK && cha.getId() == _user.getId()) || (cha.getId() == _user.getId() && _skillId == HEAL_ALL)) {
				return false;
			}
			...
			if ((_skill.getTarget().equals("attack") || _skill.getType() == L1Skill.TYPE_ATTACK)
					&& cha instanceof L1PcInstance && !_isPK && _target instanceof L1PcInstance) {
	...
	private void makeTargetList() {
		try {
			if ((_type == TYPE_LOGIN) || (_skill.getTargetTo() == L1Skill.TARGET_TO_ME
	...
	private void addMagicList(L1Character cha, boolean repetition) {
		...
		if ((_skillId == CURSE_POISON) || _skillId == CURSE_PARALYZE || _skillId == CURSE_PARALYZE2 || (_skillId == SHAPE_CHANGE)) { //
			return;
		}
		if (!(cha instanceof L1PcInstance)
	...
	private void sendGrfx(boolean isSkillAction) {
		...
		if (_user instanceof L1PcInstance) {
			...
			if ((_skillId == BONE_BREAK) || (_skillId == SMASH) || (_skillId == ARM_BREAKER)) {
				return;
			}
	...
	private boolean isTargetCalc(L1Character cha) {
		...
		if ((_skillId == RETURN_TO_NATURE && cha.getZoneType() == ZoneType.Safety) || (_skillId == FOG_OF_SLEEPING && _user.getId() == cha.getId())) {
			return false;
		}
	...
	private boolean isTargetFailure(L1Character cha) {
		...
		if ((_skillId == TURN_UNDEAD && (undeadType == 0 || undeadType == 2))
				...
				|| ((_skillId == ERASE_MAGIC || _skillId == SLOW || _skillId == MANA_DRAIN || _skillId == MASS_SLOW
						|| _skillId == ENTANGLE || _skillId == WIND_SHACKLE) && !isErase)
