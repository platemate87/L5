...
public class L1MobSkillUse {
		...
		for (int i = 0; i < getMobSkillTemplate().getSkillSize()
			...
			if (type == L1MobSkill.TYPE_PHYSICAL_ATTACK) {
				...
				if ((Config.MAX_SERVANT_SUMMONS > -1
						&& this._attacker.getServantSummonCount() > Config.MAX_SERVANT_SUMMONS) || summon(i)) {
					skillUseCountUp(i);
					return true;
				}
	...
	private boolean poly(int idx) {
		...
		for (L1PcInstance pc : L1World.getInstance().getVisiblePlayer(_attacker)) {
			if (pc.isDead() || pc.isGhost() || pc.isGmInvis() || !_attacker.glanceCheck(pc.getX(), pc.getY())) {
				continue;
			}
	...
	private boolean magicAttack(int idx) {
		...
		if (skillid > 0) {
			canUseSkill = skillUse.checkUseSkill(null, skillid, _target.getId(), _target.getX(), _target.getY(), null,
					0, L1SkillUse.TYPE_NORMAL, _attacker);
		}
		...
		if (canUseSkill) {
			...
			skillUse.handleCommands(null, skillid, _target.getId(), _target.getX(), _target.getX(), null, 0,
	...
	private boolean physicalAttack(int idx) {
		...
		if ((_attacker.getLocation().getTileLineDistance(_target.getLocation()) > range) || !_attacker.glanceCheck(_target.getX(), _target.getY())) {
			return false;
		}
		...
		if (areaHeight > 0) {
			ArrayList<L1Object> objs = L1World.getInstance().getVisibleBoxObjects(_attacker, _attacker.getHeading(),
			...
			for (L1Object obj : objs) {
				if (L1World.getInstance().findObject(obj.getId()) == null) {
					_log.error("L1MobSkillUse:physicalAttack: visibleBoxObject obj missing from L1World._allObjects!\n"
							+ obj.toString() + " Id: " + obj.getId() + " Map: " + obj.getMapId());
		...
		while (ite.hasNext()) {
			...
			if (_target.hasSkillEffect(COUNTER_BARRIER)) {
				...
				if (magic.calcProbabilityMagic(COUNTER_BARRIER)
						&& _attacker.getLocation().getTileLineDistance(_target.getLocation()) <= 2 && gfxId == 0) {
			...
			if (skillid > 0) {
				L1SkillUse skillUse = new L1SkillUse();
				canUseSkill = skillUse.checkUseSkill(null, skillid, _target.getId(), _target.getX(), _target.getY(),
						null, 0, L1SkillUse.TYPE_NORMAL, _attacker);
		...
		case L1MobSkill.CHANGE_TARGET_RANDOM:
			...
			for (L1Object obj : L1World.getInstance().getVisibleObjects(_attacker)) {
				if (obj instanceof L1PcInstance || obj instanceof L1PetInstance || obj instanceof L1SummonInstance) {
					...
					if (!getMobSkillTemplate().isTriggerDistance(idx, distance) || !_attacker.glanceCheck(cha.getX(), cha.getY()) || !_attacker.getHateList().containsKey(cha) || cha.isDead()) {
						continue;
					}
