...
public class AcceleratorChecker {
	...
	private void doFail(long interval, int rightInterval, ACT_TYPE type, boolean sendMessage) {
		String dbgInfo = String.format("\nInterval: " + interval + " RightInterval: " + rightInterval + " Morph: "
				+ _pc.getTempCharGfx() + " Weapon: " + (_pc.getCurrentWeapon() + 1) + " Act type: " + type);
		if (!_pc.isGm()) {
			_log.info(String.format("Injustice count limit exceeded for player: " + _pc.getName() + dbgInfo));
