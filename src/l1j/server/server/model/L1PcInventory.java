...
public class L1PcInventory extends L1Inventory {
	...
	public int checkAddItem(L1ItemInstance item, int count, boolean message) {
		...
		if (weight < 0 || (item.getItem().getWeight() * count / 1000) < 0 || (calcWeight240(weight) >= 240)) {
			if (message) {
				sendOverMessage(82);
			}
			return WEIGHT_OVER;
		...
		if (itemExist != null && (itemExist.getCount() + count) > MAX_AMOUNT) {
			if (message) {
				getOwner().sendPackets(new S_ServerMessage(166, "", "2,000,000,000"));
			}
			return AMOUNT_OVER;
		}
			...
			for (L1ItemInstance item : storage.loadItems(_owner.getId())) {
				...
				if (item.isEquipped()) {
					...
					if (item.getItem().getType2() == 1) {
						if (weaponEquipped) {
							_log.warn("Trying to equip extra weapon during load.");
