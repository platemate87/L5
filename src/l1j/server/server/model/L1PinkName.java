...
public class L1PinkName {
	...
	public void onAction(L1PcInstance victim) {
		...
		if (attacker == null || victim == null || !(attacker instanceof L1PcInstance) || (victim.getId() == attacker.getId())) {
			return;
		}
		...
		if (!isGmSpoofing && victim.getLawful() >= 0 && (!victim.isPinkName() || Config.DUAL_PINK)
				&& (attacker.getLawful() >= 0 || Config.CHAO_PINK) && victim.getZoneType() == ZoneType.Normal
				&& attacker.getZoneType() == ZoneType.Normal && !isNowWar) {
			if (_timer == null) {
				broadcastPink();
