...
public class L1DeathMatch {
	...
	private L1DeathMatch() {
		...
		_doorLeft = DoorTable.getInstance().createDoor(0, leftGfx, new L1Location(32638, 32884, 5153), 0, 0);
	}
		...
		if (c == 0) { // NO
			if (orderList.contains(pc)) {
				...
				if (getOrderCancelList(pc) == 4) {
					...
					pc.sendPackets(new S_ServerMessage(1267, "" + orderCancelList.get(pc.getName())));
				}
			}
		} else { // YES
			int duplicate = 0;
	...
	private void setGameEnd(int type) {
		...
		case END_STATUS_NOPLAYER:
			for (L1PcInstance pc : playerList) {
				pc.sendPackets(new S_SystemMessage(gameStartMinPlayer + " Players" + "Deathmatch will start soon"));
	...
	public void removeSkillEffect(L1PcInstance pc) {
		...
		skill.handleCommands(pc, L1SkillId.CANCELLATION, pc.getId(), pc.getX(), pc.getY(), null, 0, SKILL_TYPE[1]);
	}
		...
		public void run() {
			try {
				...
				if (checkPlayersOK()) {
					...
					for (L1Object obj : L1World.getInstance().getVisiblePoint(new L1Location(32638, 32880, getMapId()),
							3)) {
					...
					for (L1Object obj : L1World.getInstance().getVisiblePoint(new L1Location(32658, 32899, getMapId()),
							3)) {
						if (obj instanceof L1PcInstance) {
							L1Location loc = new L1Location(32625 + ThreadLocalRandom.current().nextInt(28),
									32885 + ThreadLocalRandom.current().nextInt(28), getMapId());
			...
			if (getGameStatus() == STATUS_PLAYING) {
				for (L1PcInstance pc : playerList) {
					if (pc.getMapId() == getMapId()) {
						L1ItemInstance item = ItemTable.getInstance().createItem(ISSUE_ITEM);
		...
		public void run() {
			if (_door.getOpenStatus() == ActionCodes.ACTION_Open) {
				for (L1Object obj : L1World.getInstance()
						.getVisiblePoint(new L1Location(_door.getX(), _door.getY(), _door.getMapId()), 0)) {
					if (obj instanceof L1PcInstance) {
						L1Location loc = new L1Location(32625 + ThreadLocalRandom.current().nextInt(28),
								32885 + ThreadLocalRandom.current().nextInt(28), getMapId());
						while (!loc.getMap().isPassable(loc.getX(), loc.getY())) {
				...
				for (L1Object obj : L1World.getInstance()
						.getVisiblePoint(new L1Location(_door.getX() + 1, _door.getY(), _door.getMapId()), 0)) {
					if (obj instanceof L1PcInstance) {
						L1Location loc = new L1Location(32625 + ThreadLocalRandom.current().nextInt(28),
								32885 + ThreadLocalRandom.current().nextInt(28), getMapId());
						while (!loc.getMap().isPassable(loc.getX(), loc.getY())) {
