		...
		try {
			...
			while (rs.next()) {
				...
				for (int i = 0; i < count; i++) {
					L1TrapInstance trap = new L1TrapInstance(IdFactory.getInstance().nextId(), trapTemp, loc, rndPt,
							span);
	...
	private void resetTimer() {
		synchronized (this) {
			for (L1TrapInstance trap : _allTraps) {
				...
				try {
					trap.set_trapFuture(
							GeneralThreadPool.getInstance().schedule(new TrapSpawnTimer(trap), trap.getSpan()));
				} catch (Exception e) {
					_log.error("", e);
				}
			}
