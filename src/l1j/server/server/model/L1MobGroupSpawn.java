...
public class L1MobGroupSpawn {
	...
	private static final Logger _log = LoggerFactory.getLogger(L1MobGroupSpawn.class.getName());
	...
	private boolean canSpawn(L1NpcInstance mob) {
		if (mob.getMap().isInMap(mob.getLocation()) && mob.getMap().isPassable(mob.getLocation())) {
			if (_isRespawnScreen || (L1World.getInstance().getVisiblePlayer(mob).size() == 0)) {
				return true;
			}
		}
		return false;
