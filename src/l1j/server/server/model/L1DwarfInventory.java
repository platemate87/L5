...
public class L1DwarfInventory extends L1Inventory {
	...
	public void loadItems() {
		...
		try {
			...
			pstm = con.prepareStatement("SELECT * FROM character_warehouse WHERE account_name = ? ORDER BY item_name");
	...
	public void insertItem(L1ItemInstance item) {
		...
		try {
			...
			pstm = con.prepareStatement(
					"INSERT INTO character_warehouse SET id = ?, account_name = ?, item_id = ?, item_name = ?, count = ?, is_equipped=0, enchantlvl = ?, is_id = ?, durability = ?, charge_count = ?, remaining_time = ?, last_used = ?, bless = ?, attr_enchant_kind = ?, attr_enchant_level = ?, m_def = ?, add_hp = ?, add_mp = ?, add_hpr = ?, add_mpr = ?, add_sp = ?, defense_water = ?, defense_wind = ?, defense_fire = ?, defense_earth = ?");
		...
		try {
			...
			for (String account : accountList) {
				...
				if (temp.isStackable()) {
					...
					pstm = con.prepareStatement(
							"INSERT INTO character_warehouse SET id = ?, account_name = ?, item_id = ?, item_name = ?, count = ?, is_equipped=0, enchantlvl = ?, is_id = ?, durability = ?, charge_count = ?, remaining_time = ?");
					pstm.setInt(1, item.getId());
	...
	private static Logger _log = LoggerFactory.getLogger(L1DwarfInventory.class.getName());
