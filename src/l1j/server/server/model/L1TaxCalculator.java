...
public class L1TaxCalculator {
	...
	/**
	 * @param merchantNpcId target stores calculation NPCID
	 */
	...
	public int calcCastleTaxPrice(L1ShopItem item, int count, boolean subtractOtherTaxes) {
		...
		for (int i = 0; i < count; i++) {
			castleTaxPrice += (int) ((item.getPrice() * Config.RATE_SHOP_SELLING_PRICE) * _taxRatesCastle / 100);
		}
		...
		return castleTaxPrice - calcNationalTaxPrice(item, count) - calcDiadTaxPrice(item, count);
	}
	...
	public int calcNationalTaxPrice(L1ShopItem item, int count) {
		...
		for (int i = 0; i < count; i++) {
			taxCastle += (int) ((item.getPrice() * Config.RATE_SHOP_SELLING_PRICE) * _taxRatesCastle / 100);
		}
	...
	public int calcTownTaxPrice(L1ShopItem item, int count) {
		...
		for (int i = 0; i < count; i++) {
			townTaxPrice += (int) ((item.getPrice() * Config.RATE_SHOP_SELLING_PRICE) * _taxRatesTown / 100);
		}
	...
	public int calcWarTaxPrice(L1ShopItem item, int count) {
		...
		for (int i = 0; i < count; i++) {
			warTaxPrice += (int) ((item.getPrice() * Config.RATE_SHOP_SELLING_PRICE) * _taxRatesWar / 100);
		}
	...
	public int calcDiadTaxPrice(L1ShopItem item, int count) {
		...
		for (int i = 0; i < count; i++) {
			diadCastleTaxPrice += (int) ((item.getPrice() * Config.RATE_SHOP_SELLING_PRICE) * _taxRatesCastle / 100);
		}
