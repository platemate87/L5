...
public class L1MapLimiter implements Runnable {
	...
	private static HashMap<Integer, L1MapLimiter> _dataMap = new HashMap<>();
	...
	public static void reload() {
		HashMap<Integer, L1MapLimiter> dataMap = new HashMap<>();
		loadXml(dataMap);
		_dataMap = dataMap;
	}
				...
				if (effect.getTime() > 0) {
					...
					if (enterTime <= effect.getDelay()) {
						...
						// Remaining time is x seconds
							...
							|| ((enterTime <= 600) && (enterTime % 60 == 0))) { // every 10 minutes, or 10 minutes or
																				// less
						if (h > 0) {
							if (m > 0) {
