...
public class Maps {
	...
	private static final int[] INNS = new int[] { 16384, 16896, 17408, 17920, 18432, 18944, 19968, 19456, 20480, 20992,
			21504, 22016, 22528, 23040, 23552, 24064, 24576, 25088 };
	...
	public static boolean atMotherTree(L1Character character) {
		return character.getMapId() == ADEN && character.getLocation().isInScreen(MotherTree);
	}

	public static boolean atSilveriaCenter(L1Character character) {
		return character.getMapId() == SILVERIA && character.getLocation().isInScreen(TownCenter);
	}

	public static boolean atBehimousCenter(L1Character character) {
		return character.getMapId() == BEHIMOUS && character.getLocation().isInScreen(TownCenter);
	}
}
