...
public class L1V1Map extends L1Map {
	 ...
	 * Readability is a large drop in child imitating not so good. Mob, and traffic
	 * on the tiles is impossible to object if a bit flag indicating
	 */
	...
	public String toCsv() throws IOException {
		...
		ArrayList<StringBuilder> mappings = new ArrayList<>();
	...
	public boolean isExistDoor(int x, int y) {
		for (L1DoorInstance door : DoorTable.getInstance().getDoorList()) {
			if ((_mapId != door.getMapId()) || (door.getOpenStatus() == ActionCodes.ACTION_Open) || door.isDead()) {
				continue;
			}
