...
public class L1GuardInstance extends L1NpcInstance {
	...
	private static final Map<Integer, Integer> guardsToCastles = new HashMap<>();

	private static final Map<Integer, String> guardsToText = new HashMap<>();
	...
	public void searchTarget() {
		...
		for (L1PcInstance pc : L1World.getInstance().getVisiblePlayer(this)) {
			if (pc.getCurrentHp() <= 0 || pc.isDead() || pc.isGm() || pc.isGhost() || pc.isGmInvis()) {
				continue;
			}
			if (!pc.isInvisble() || getNpcTemplate().is_agrocoi()) {
				...
				if (pc.isWanted() && (!guardsToCastles.containsKey(npcid)
						|| (guardsToCastles.containsKey(npcid) && !checkHasCastle(pc, guardsToCastles.get(npcid))))) {
					_hateList.add(pc, 0);
					_target = pc;
					return;
				}
	...
	public void onTalkAction(L1PcInstance player) {
		...
		case 70600:
		case 70986:
			htmlid = checkHasCastle(player, guardsToCastles.get(npcid)) ? "orckeeper" : "orckeeperop";
			break;
		case 60514:
		case 60560:
		case 60552:
		...
		if (htmlid != null)
			...
			player.sendPackets(new S_NPCTalkReturn(talking, objid, player.getLawful() < -1000 ? 2 : 1));
	}
