...
public class L1MonsterInstance extends L1NpcInstance {
	...
	private static Logger _log = LoggerFactory.getLogger(L1MonsterInstance.class.getName());
	...
	public void searchTarget() {
		...
		if (!base.is_agro() && !base.is_agrososc() && !base.is_agrochao() && base.is_agrogfxid1() < 0
				&& base.is_agrogfxid2() < 0)
			return;

		for (L1PcInstance pc : L1World.getInstance().getVisiblePlayer(this)) {

			// NOTE: Don't remove non-aggro to shopmode
			if (pc.getCurrentHp() <= 0 || pc.isDead() || pc.isGm() || pc.isGmInvis() || pc.isGhost()
					|| pc.isPrivateShop()) {
				continue;
			}
			...
			if (id == 45600) {
				if (pc.isCrown() || pc.isDarkelf() || playerGfx != pc.getClassId()) {
					targetPlayer = pc;
					break;
				}
			}

			if ((base.getKarma() < 0 && pc.getKarmaLevel() >= 1) || (base.getKarma() > 0 && pc.getKarmaLevel() <= -1)) {
				continue;
			}

			if (playerGfx == 6034 && base.getKarma() < 0
					|| playerGfx == 6035 && (base.getKarma() > 0 || id == 46070 || id == 46072)) {
				continue;
			}

			if (!base.is_agro() && !base.is_agrososc() && base.is_agrogfxid1() < 0 && base.is_agrogfxid2() < 0) {
	...
	public void onTalkAction(L1PcInstance pc) {
		...
		pc.sendPackets(new S_NPCTalkReturn(talking, getId(), pc.getLawful() < -1000 ? 2 : 1));
	}
		...
		public void run() {
			try {
				...
				if (getServantMaster() instanceof L1NpcInstance) {
					...
					// if it is something a GM has spawned or a boss, then allow respawning of
					// servants
				...
				if (Config.ALT_BOSS_EVENT && BossEventController.getInstance().getBossIds().contains(getNpcId())) {
					...
					world.broadcastServerMessage(
							String.format("\\fR[******] %s has protected Aden by slaying %s!", name, getName()));
				}

				startDeleteTimer();

				// Note: this will not count bosses spawned by GMs.
				if (_lastAttacker instanceof L1PcInstance && getSpawn() instanceof L1BossSpawn) {
					try {
						...
						pstm = con.prepareStatement(
								"INSERT INTO boss_kills (npcid, boss_name, locx, locy, mapid, killer_name, killer_objid, clan_name, kill_date) VALUES(?,?,?,?,?,?,?,?,?)");
						...
						_log.error("Error occurred logging boss kill!: " + e.getLocalizedMessage(), e);
					}
				}
			} catch (Exception e) {
	...
	private void hide() {
		...
		case 45455:
			...
			break;
		case 45682:
			hideHelper(3, 50, 20, HIDDEN_STATUS_SINK, new S_DoActionGFX(getId(), ActionCodes.ACTION_AntharasHide));
			break;
		case 45067:
		case 45264:
		case 45452:
	...
	public void initHideForMinion(final L1NpcInstance leader) {
		int npcid = getNpcTemplate().get_npcId();
		if (leader.getHiddenStatus() == HIDDEN_STATUS_SINK || leader.getHiddenStatus() == HIDDEN_STATUS_FLY
