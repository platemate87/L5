...
public class L1Magic {
	...
	private static final int[] NOT_IN_SAFE_ZONES = new int[] { WEAPON_BREAK, SLOW, CURSE_PARALYZE, MANA_DRAIN, DARKNESS,
			WEAKNESS, DISEASE, DECAY_POTION, MASS_SLOW, ENTANGLE, ERASE_MAGIC, EARTH_BIND, AREA_OF_SILENCE,
			WIND_SHACKLE, STRIKER_GALE, SHOCK_STUN, FOG_OF_SLEEPING, ICE_LANCE, FREEZING_BLIZZARD, CANCELLATION,
			POLLUTE_WATER, CURSE_POISON, ELEMENTAL_FALL_DOWN, CURSE_BLIND, RETURN_TO_NATURE, DARK_BLIND, SILENCE,
			FREEZING_BREATH, RESIST_FEAR, GUARD_BRAKE, HORROR_OF_DEATH };
	...
	public boolean calcProbabilityMagic(int skillId) {
		...
		if (isMissingSkillEffect() || !checkZone(skillId)) {
			return false;
		}
		if (skillId == CANCELLATION) {
			if (_calcType == PC_PC && _pc != null && _targetPc != null) {
				...
				if (_pc.getZoneType() == ZoneType.Safety || _targetPc.getZoneType() == ZoneType.Safety) {
					return false;
				}
			}
	...
	private int calcProbability(int skillId) {
		...
		if (skillId == ELEMENTAL_FALL_DOWN || skillId == RETURN_TO_NATURE || skillId == ENTANGLE
			...
			probability = skill.getProbabilityValue() * (_attacker.getLevel() / Math.max(1, defenseLevel))
			...
			int diceCount = _attacker.getMagicBonus() + _attacker.getMagicLevel();
			if (_calcType == PC_PC || _calcType == PC_NPC) {
	...
	private static boolean tryCounterMirror(L1PcInstance target, int type, L1PcInstance pcAttacker,
			L1NpcInstance npcAttacker, int damage) {
		if (!target.hasSkillEffect(COUNTER_MIRROR) || target.getWis() <= ThreadLocalRandom.current().nextInt(100)
	 ...
	 * arises in pve situations, e.g. attacking Chaos. TODO: find a better name.
	 * TODO: L1Attack and L1Magic both need this check. Duplicating code to avoid
	 * lots of casting. (Need to test whether it's really worth it.)
	 */
	...
	private int calcMagicDiceDamage(int skillId) {
		...
		int charaIntelligence = Math.max(_attacker.getInt() + spByItem - INT_ADJUSTMENT, 1);

		double attrDeffence = calcAttrResistance(skill.getAttr());
		double coefficient = Math.max((1.0 - attrDeffence + charaIntelligence * 3.0 / 32.0), 0);
		...
		if (_calcType == PC_PC || _calcType == PC_NPC) {
			if (skill.getSkillLevel() <= CRIT_LEVEL_LIMIT && ThreadLocalRandom.current().nextInt(100)
					+ 1 <= BASE_CRIT_RATE + _pc.getOriginalMagicCritical()) {
