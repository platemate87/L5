...
public class L1CastleLocation {
	...
	private static final L1MapArea KentArea = new L1MapArea(33089, 32717, 33219, 32827, 4);
	private static final short KENT_INNER_CASTLE_MAP = 15;

	private static final short OT_MAP = 4;
	private static final L1Location OTTower = new L1Location(32800, 32290, OT_MAP);
	private static final L1MapArea OTArea = new L1MapArea(32750, 32250, 32850, 32350, OT_MAP);

	private static final L1Location WWTower = new L1Location(32675, 33408, 4);
	private static final L1MapArea WWArea = new L1MapArea(32571, 33350, 32721, 33460, 4);
	private static final short WW_INNER_CASTLE_MAP = 29;

	private static final L1Location GiranTower = new L1Location(33631, 32678, 4);
	private static final L1MapArea GiranArea = new L1MapArea(33559, 32615, 33686, 32755, 4);
	...
	private static final L1MapArea HeineArea = new L1MapArea(33458, 33315, 33583, 33490, 4);
	private static final short HEINE_INNER_CASTLE_MAP = 64;

	private static final short DOWA_MAP = 66;
	private static final L1Location DowaTower = new L1Location(32828, 32818, DOWA_MAP);
	private static final L1MapArea DowaArea = new L1MapArea(32755, 32790, 32870, 32920, DOWA_MAP);

	private static final L1Location AdenTower = new L1Location(34090, 33260, 4);
	private static final L1MapArea AdenArea = new L1MapArea(34007, 33172, 34162, 33332, 4);
	...
	private static final L1Location DiadTower = new L1Location(33033, 32895, 320);
	private static final L1MapArea DiadArea = new L1MapArea(32888, 32839, 33070, 32953, 320);
	...
	public static int[] getWarArea(int castleId) {
		...
		if (area == null)
			return new int[5];
		return new int[] { area.getLeft(), area.getRight(), area.getTop(), area.getBottom(), area.getMapId() };
	}

	private static final Map<Integer, int[]> castleLocations = new HashMap<>();

	static {
		castleLocations.put(KENT_CASTLE_ID, new int[] { 32731, 32810, KENT_INNER_CASTLE_MAP });
		castleLocations.put(OT_CASTLE_ID, new int[] { 32800, 32277, OT_MAP });
		castleLocations.put(WW_CASTLE_ID, new int[] { 32730, 32814, WW_INNER_CASTLE_MAP });
		castleLocations.put(GIRAN_CASTLE_ID, new int[] { 32724, 32827, GIRAN_INNER_CASTLE_MAP });
		castleLocations.put(HEINE_CASTLE_ID, new int[] { 32568, 32855, HEINE_INNER_CASTLE_MAP });
		castleLocations.put(DOWA_CASTLE_ID, new int[] { 32853, 32810, DOWA_MAP });
		castleLocations.put(ADEN_CASTLE_ID, new int[] { 32892, 32572, ADEN_INNER_CASTLE_MAP });
		castleLocations.put(DIAD_CASTLE_ID, new int[] { 32733, 32985, DIAD_INNER_CASTLE_MAP });
	}
