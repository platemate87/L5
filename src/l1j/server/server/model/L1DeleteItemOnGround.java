...
public class L1DeleteItemOnGround {
	...
	private HashSet<L1Object> checkeditems = new HashSet<>();
	...
	private void deleteItem() {
		...
		for (Iterator<L1Object> iterator = checkeditems.iterator(); iterator.hasNext();) {
			...
			if (L1World.getInstance().getObject().contains(previouslycheckeditem)) {
				...
				// in case...
				if ((item.getX() == 0 && item.getY() == 0) || L1HouseLocation.isInHouse(item.getX(), item.getY(), item.getMapId()) || (L1World.getInstance().getInventory(item.getX(), item.getY(), item.getMapId()) == null)) {
				...
				// anyone
				List<L1PcInstance> players = L1World.getInstance().getVisiblePlayer(item,
						Config.ALT_ITEM_DELETION_RANGE);

				if (players.isEmpty()) { //
					L1Inventory groundInventory = L1World.getInstance().getInventory(item.getX(), item.getY(),
							item.getMapId());
					try {
		...
		for (L1Object obj : L1World.getInstance().getObject()) {
			...
			if ((item.getX() == 0 && item.getY() == 0) || (item.getItem().getItemId() == 40515) || L1HouseLocation.isInHouse(item.getX(), item.getY(), item.getMapId())) { // Hideout in
				continue;
			}
