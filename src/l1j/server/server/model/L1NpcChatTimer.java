...
public class L1NpcChatTimer implements Runnable {
	private static final Logger _log = LoggerFactory.getLogger(L1NpcChatTimer.class.getName());
	...
	public L1NpcChatTimer(L1NpcInstance npc, L1NpcChat chat) {
		...
		chatIds = new String[] { chat.getChatId1(), chat.getChatId2(), chat.getChatId3(), chat.getChatId4(),
				chat.getChatId5(), };
	}
	...
	public void run() {
		try {
			if (_npc == null || _npc._destroyed || _npc.getHiddenStatus() != L1NpcInstance.HIDDEN_STATUS_NONE) {
				return;
			}
	...
	private void chat(L1NpcInstance npc, int chatTiming, String chatId, boolean isShout, boolean isWorldChat) {
		if (npc.isDead() && (chatTiming == L1NpcInstance.CHAT_TIMING_APPEARANCE
				|| chatTiming == L1NpcInstance.CHAT_TIMING_HIDE)) {
			return;
		}
