...
public class L1EffectSpawn {
	private static final Logger _log = LoggerFactory.getLogger(L1EffectSpawn.class.getName());
	...
	public L1EffectInstance spawnEffect(int npcId, int time, int locX, int locY, short mapId, L1PcInstance user,
			int skiiId) {
		...
		String className = (new StringBuilder()).append("l1j.server.server.model.Instance.").append(template.getImpl())
	...
	public void doSpawnFireWall(L1Character cha, int targetX, int targetY) {
		...
		int duration = SkillTable.getInstance().findBySkillId(FIRE_WALL).getBuffDuration();
