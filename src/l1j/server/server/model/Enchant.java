...
public class Enchant {
	...
	private static final int[] IvoryTowerWeapons = new int[] { 7, 35, 48, 73, 105, 120, 147, 156, 174, 175, 224 };

	private static final int[] IvoryTowerArmor = new int[] { 20028, 20082, 20126, 20173, 20206, 20232, 21138, 21140,
			21141 };

	private static final int[] ShadowFidWeaponIds = new int[] { 550012, 550061, 550086, 550134, 550160, 550284, 550285,
			550286 };
	...
	public static void elementEnchant(final L1PcInstance player, final L1ItemInstance scroll,
			final L1ItemInstance weapon) {
		if (!Config.ELEMENTAL_ENCHANTING) {
			player.sendPackets(new S_SystemMessage("Elemental enchanting is disabled on this server."));
			return;
		}
		...
		if (player.isGm() && player.getAccessLevel().getLevel() >= FULL_GM
			...
			player.sendPackets(new S_ServerMessage(161, weapon.getLogName(), "$245", "$247"));
	...
	public static void enchantAccessory(final L1PcInstance player, final L1ItemInstance scroll,
			final L1ItemInstance accessory) {
		if (!Config.ACCESSORY_ENCHANTING) {
			player.sendPackets(new S_SystemMessage("Accessory enchanting is disabled on this server."));
			return;
		}
		...
		if (accessory.getBless() >= 128 || accessoryBase.getType2() != 2 || accessoryBase.getType() < 8
				|| accessoryBase.getType() > 12 || accessoryBase.getGrade() == -1) {
			player.sendPackets(CantEnchant);
			return;
		}
	...
	private static void failureEnchant(final L1PcInstance player, final L1ItemInstance item) {
		String name = item.getName();
		int enchantLevel = item.getEnchantLevel();
		String pm = enchantLevel > 0 ? "+" : "";
		String s = (!item.isIdentified() || enchantLevel == 0) ? name : pm + enchantLevel + " " + name;
		...
		if (Config.LOGGING_ENCHANT_FAIL != 0
				&& Config.LOGGING_ENCHANT_FAIL <= (item.getEnchantLevel() - item.getItem().get_safeenchant())) {
			LogEnchantFail failLog = new LogEnchantFail();
			failLog.storeLogEnchantFail(player, item);
		}
	}

	private static void successEnchant(final L1PcInstance player, final L1ItemInstance item, final int change) {
		L1Item baseItem = item.getItem();
		boolean isWeapon = baseItem.getType2() == 1;
		boolean isArmor = baseItem.getType2() == 2;

		if (!isArmor && !isWeapon) {
			_log.warn("Enchant: trying to enchant something" + " besides a weapon or piece of armor.");
			return;
		}
		...
		if (isWeapon && Config.LOGGING_WEAPON_ENCHANT != 0) {
			if (safeEnchant == 0 || newEnchantLevel >= Config.LOGGING_WEAPON_ENCHANT) {
				LogEnchantTable logenchant = new LogEnchantTable();
				logenchant.storeLogEnchant(player.getId(), item.getId(), enchantLevel, newEnchantLevel);
			}
		}
		if (isArmor && Config.LOGGING_ARMOR_ENCHANT != 0) {
			if (safeEnchant == 0 || newEnchantLevel >= Config.LOGGING_ARMOR_ENCHANT) {
				LogEnchantTable logenchant = new LogEnchantTable();
				logenchant.storeLogEnchant(player.getId(), item.getId(), enchantLevel, newEnchantLevel);
			}
		}
	...
	private static int getEnchantChange(final L1ItemInstance item, final int scrollId) {
		if (scrollId == L1ItemId.C_SCROLL_OF_ENCHANT_ARMOR || scrollId == L1ItemId.C_SCROLL_OF_ENCHANT_WEAPON)
			return -1;
		if (scrollId == L1ItemId.B_SCROLL_OF_ENCHANT_ARMOR || scrollId == L1ItemId.B_SCROLL_OF_ENCHANT_WEAPON
	...
	public static void enchantIvoryTowerWeapon(final L1PcInstance player, final L1ItemInstance scroll,
			final L1ItemInstance weapon) {
		if (weapon == null || weapon.getItem().getType2() != 1 || weapon.getBless() >= 128) {
			player.sendPackets(CantEnchant);
			return;
	...
	public static void enchantIvoryTowerArmor(final L1PcInstance player, final L1ItemInstance scroll,
		...
		if (armor.getItem().getType2() != 2 || armor.getBless() >= 128 || !IntArrays.sContains(IvoryTowerArmor, armorId)) {
			player.sendPackets(CantEnchant);
			return;
		}
	...
	private static boolean scrollMatchesWeapon(final int weaponId, final int scrollId) {
		if (IntArrays.sContains(IvoryTowerWeapons, weaponId))
			return false;
	...
	private static boolean scrollMatchesArmor(final int armorId, final int scrollId) {
	...
	public static void enchantWeapon(final L1PcInstance player, final L1ItemInstance scroll,
			final L1ItemInstance weapon) {
		...
		if (weapon.getItem().getType2() != 1 || safeEnchant < 0 || weapon.getBless() >= 128 || !scrollMatchesWeapon(weaponId, scrollId)) {
			player.sendPackets(CantEnchant);
			return;
		}
		...
		Result result = player.isGm() && player.getAccessLevel().getLevel() >= FULL_GM ? Result.Success
		...
		case Success:
			...
			break;
		case Unchanged:
			player.sendPackets(new S_ServerMessage(160, weapon.getLogName(), "$245", "$248"));
			break;
		case Failure:
	...
	private static Result enchantWeapon(final int scrollId, final int enchantLevel, final int safeEnchant) {
		...
		return roll < enchantChance ? Result.Success
				: roll < enchantChance * 2 && enchantLevel >= 9 ? Result.Unchanged : Result.Failure;
	}

	public static void enchantArmor(final L1PcInstance player, final L1ItemInstance scroll,
			final L1ItemInstance armor) {
		int scrollId = scroll.getItem().getItemId();
		if (armor == null) {
			player.sendPackets(CantEnchant);
		...
		int safeEnchant = armor.getItem().get_safeenchant();
		int armorId = armor.getItem().getItemId();
		if (armor.getItem().getType2() != 2 || safeEnchant < 0 || armor.getBless() >= 128 || !scrollMatchesArmor(armorId, scrollId)) {
			player.sendPackets(CantEnchant);
			return;
		}
		...
		case Success:
			...
			break;
		case Unchanged:
			player.sendPackets(new S_ServerMessage(160, armor.getLogName(), "$252", "$248"));
			break;
		case Failure:
	...
	private static Result enchantArmor(final int scrollId, final int enchantLevel, final int safeEnchant) {
		...
		int enchantChance = (100 + divisor * Config.ENCHANT_CHANCE_ARMOR) / (enchantLevel >= 9 ? divisor * 2 : divisor);
		return roll < enchantChance ? Result.Success
				: roll < enchantChance * 2 && enchantLevel >= 9 ? Result.Unchanged : Result.Failure;
	}
}
