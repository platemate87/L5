...
public class L1ClassId {
	...
	public static String getSex(int classId) {
		...
		_log.error("Unrecognized classId in #L1ClassId.getSex(). Check db.");
		throw new IllegalStateException("Unrecognized character class id.");
	}

	private static boolean isMale(int classId) {
		return classId == CLASSID_PRINCE || classId == CLASSID_KNIGHT_MALE || classId == CLASSID_ELF_MALE
				|| classId == CLASSID_WIZARD_MALE || classId == CLASSID_DARK_ELF_MALE
				|| classId == CLASSID_DRAGON_KNIGHT_MALE || classId == CLASSID_ILLUSIONIST_MALE;
	}

	private static boolean isFemale(int classId) {
		return classId == CLASSID_PRINCESS || classId == CLASSID_KNIGHT_FEMALE || classId == CLASSID_ELF_FEMALE
				|| classId == CLASSID_WIZARD_FEMALE || classId == CLASSID_DARK_ELF_FEMALE
				|| classId == CLASSID_DRAGON_KNIGHT_FEMALE || classId == CLASSID_ILLUSIONIST_FEMALE;
	}
	...
	public static String getClass(int classId, boolean ignoreErrors) {
		...
		if (!ignoreErrors)
			_log.error("Unrecognized classId in #L1ClassId.getClass(). Check db.");
		throw new IllegalStateException("Unrecognized character class id.");
	}
