...
public class S_Ranking extends ServerBasePacket {
	...
	private static List<Integer> _emptyPages = Arrays
			.asList(new Integer[] { 25, 30, 31, 32, 34, 40, 41, 42, 48, 49, 50 });
	...
	private void buildPacket(L1PcInstance pc, int number) {
		...
		String[] titles = {
				...
				"Overall Lvl Ranking", "Royal Lvl Ranking", "Knight Lvl Ranking", "Elf Lvl Ranking", "Mage Lvl Ranking",
				"Dark Elf Lvl Ranking", "DK Lvl Ranking", "Illu. Lvl Ranking",

				// Pvp Ranking Titles
				"Overall PvP Ranking", "Royal PvP Ranking", "Knight PvP Ranking", "Elf PvP Ranking", "Mage PvP Ranking",
				"Dark Elf PvP Ranking", "DK PvP Ranking", "Illu. PvP Ranking",
		...
		RankingsController.RankType[] rankEnumAssoc = { RankType.LEVEL, RankType.LEVELCLASS, RankType.LEVELCLASS,
				RankType.LEVELCLASS, RankType.LEVELCLASS, RankType.LEVELCLASS, RankType.LEVELCLASS, RankType.LEVELCLASS,
				RankType.PVP, RankType.PVPCLASS, RankType.PVPCLASS, RankType.PVPCLASS, RankType.PVPCLASS,
				RankType.PVPCLASS, RankType.PVPCLASS, RankType.PVPCLASS,
		...
		if (number > 34) {
			if (rankType == RankType.BOSSDAILY) {// for boss rankings, just list them all on one page
				...
				writeS(String.format("\r\n\r\n%s\r\n\r\n%s\r\n\r\n%s\r\n\r\n%s", dailyRank, weeklyRank, monthlyRank,
						yearlyRank));
			...
			String weeklyRank = RankingsController.getRank(pc, RankType.PLEDGEBOSSWEEKLY).replace(".",
					" (Boss Weekly).");
			String monthlyRank = RankingsController.getRank(pc, RankType.PLEDGEBOSSMONTHLY).replace(".",
					" (Boss Monthly).");
			String yearlyRank = RankingsController.getRank(pc, RankType.PLEDGEBOSSYEARLY).replace(".",
					" (Boss Yearly).");

			writeS(String.format("%s\r\n\r\n%s\r\n\r\n%s\r\n\r\n%s\r\n\r\n%s\r\n\r\n%s", levelRank, pvpRank, dailyRank,
					weeklyRank, monthlyRank, yearlyRank));
		} else {
