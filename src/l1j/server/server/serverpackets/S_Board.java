...
public class S_Board extends ServerBasePacket {
	...
	private void buildPacket(L1NpcInstance board, int number, L1PcInstance pc) {
			...
			if (!bugBoard && !rankingBoard || pc == null) {
				...
				if (pc.isGm())
					...
					pstm = con.prepareStatement(
							"SELECT * FROM bugs where charname = ?  AND resolved != 1 order by id desc");
		...
		/*
		 * writeC(Opcodes.S_OPCODE_BOARD); writeD(board.getId()); if (number == 0) {
		 * writeD(0x7FFFFFFF); } else { writeD(number); }
		 *
		 * writeC(count);
		 *
		 * if (number == 0) { writeC(0); writeH(300); }
		 */
