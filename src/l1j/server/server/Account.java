...
public class Account {
	...
	public ArrayList<BanInfo> getBanHistory() {
		ArrayList<BanInfo> banHistory = new ArrayList<>();
		Connection con = null;
		PreparedStatement pstm = null;
		ResultSet rs = null;
		...
		try {
			...
			rs = pstm.executeQuery();
			while (rs.next()) {
				banHistory.add(new BanInfo(rs.getString("action"), rs.getString("target_account"),
						rs.getString("actioner_account"), rs.getString("message"), rs.getTimestamp("timestamp")));
			}
		} catch (SQLException e) {
			_log.error(e.getLocalizedMessage(), e);
		} finally {
	...
	public boolean validatePassword(final String rawPassword) {
		...
		try {
			_isValid = _password.equals(makeSHA256(Config.PASSWORD_SALT + rawPassword + makeMD5(_name)));// Ssargon
																											// change
			if (_isValid) {
				_password = null;
				...
				if (_isValid) { // If its old scheme password
					change_password(makeSHA256(Config.PASSWORD_SALT + rawPassword + makeMD5(_name))); // Make new style
																										// password
				}
			}
			return _isValid;
