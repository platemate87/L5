...
public class JailController implements Runnable {
	...
	private HashMap<String, JailInfo> _jailedUsers = new HashMap<>();
	...
	private void getJailedUsers(Long lastCheck) {
		...
		try {
			...
			while (rs.next()) {
				...
				this._jailedUsers.put(username, new JailInfo(rs.getString("jailer"), rs.getString("message"),
						rs.getTimestamp("unjail").getTime()));
			}
		} catch (Exception ex) {
	...
	private void setUnjailStatus(String user) {
		...
		try {
			...
			pstm = con.prepareStatement(
					"UPDATE `log_jail` SET `status` = ?, modified = NOW() WHERE jailed_char_name = ?");
