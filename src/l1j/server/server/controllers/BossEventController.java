...
public class BossEventController implements Runnable {
	...
	SecureRandom rand;
	ArrayList<Integer> bossIds = new ArrayList<>();
	HashMap<String, Integer> ipsHit = new HashMap<>();
		...
		try {
			while (true) {
				...
				if (day != lastDayRun && Config.ALT_BOSS_EVENT_DAILY_RESET
						&& dateFormat.parse(dateFormat.format(rightNow.getTime()))
								.after(dateFormat.parse(Config.ALT_BOSS_EVENT_RESET_TIME + ":00"))) {
					lastDayRun = day;
					ipsHit.clear();
				}

				// if we haven't spawned a boss this hour, set the sleep time to some random
				// interval
				// that will cause them to spawn within this hour
				if (lastHourRun != hour && !spawnBoss) {
					int minute = 57 - rightNow.get(Calendar.MINUTE); // don't spawn within the last 3 minutes of the
																		// hour
					...
					if (players.size() > 0) { // really only needed for testing, but good to have
						...
						Thread.sleep(rand.nextInt(180000) + 120000); // wait 2-5 minutes for players to get to the
																		// correct map
						...
						while (spawnedBosses < numBossesToSpawn) {
							...
							if (player == null || player.getMapId() != 4
									|| !player.getMap().isNormalZone(player.getLocation()) || player.getLevel() < 55
									|| player.isGm() || player.isDead()) {
								continue;
							}
							...
							world.broadcastServerMessage(
									String.format("\\fR[******] %s has appeared!", boss.get_name()));
							_log.info(String.format("The Boss Event has spawned %s on player %s!", boss.get_name(),
									player.getName()));
