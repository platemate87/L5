...
public class HomeTownTimeController {
	private static Logger _log = LoggerFactory.getLogger(HomeTownTimeController.class.getName());
	...
	private static String totalContribution(int townId) {
		...
		try {
			...
			pstm4 = con.prepareStatement(
					"UPDATE characters SET Pay = Contribution * ?, Contribution = 0 WHERE HomeTownID = ?");
			pstm4.setDouble(1, contributionUnit);
			pstm4.setInt(2, townId);
			pstm4.execute();
			pstm5 = con.prepareStatement(
					"UPDATE town SET leader_id = ?, leader_name = ?, town_fix_tax = 0 WHERE town_id = ?");
