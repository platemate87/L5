...
public class WarTimeController implements Runnable {
	...
	private WarTimeController() {
		for (int i = 0; i < _l1castle.length; i++) {
			...
			if (CheckWarTime.getInstance().isActive(_l1castle[i].getId()) && nowTime.after(oldTime)) {
	...
	private void clearCastle(int castleId) {
		...
		Collection<L1PcInstance> onlineAndOfflinePlayers = new ArrayList<>();
		...
		for (L1PcInstance pc : onlineAndOfflinePlayers) {
		...
		for (int i = 0; i < 8; i++) {
			...
			if (_war_start_time[i].before(getRealTime()) && _war_end_time[i].after(getRealTime())) {
				...
				if (_is_now_war[i]) {
					...
					clearCastle(castleId);
					_is_now_war[i] = false;
					L1World.getInstance().broadcastPacketToAll(new S_PacketBox(S_PacketBox.MSG_WAR_END, castleId));
