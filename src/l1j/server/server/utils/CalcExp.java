...
//	private static final double PetPenalty = .12;
	...
	/**
	 * Returns a two element array containing the total experience hate and total
	 * alternate (karma and lawful hate). Warning: prunes the parameter lists!
	 */
...
//	}
	...
	/**
	 * Partitions a list of players, summons, and pets into groups divided by party,
	 * where pets and summons are in the same group as their masters. UGLY: Bite the
	 * bullet and do the searching instead? Makes for an uglier overall procedure
	 * but removes most of the extraneous wrapping.
	 */
...
//	}
	 ...
	 */
	public static void calcExp(L1PcInstance l1pcinstance, int targetid, ArrayList<L1Character> acquisitors,
			ArrayList<Integer> hateList, int exp) {
		...
		if ((acquisitors.size() != hateList.size()) || l1object == null || npc instanceof L1PetInstance || npc instanceof L1SummonInstance) {
			return;
		}
	...
	private static double getRoyalBonus(final L1PcInstance player) {
		if (!player.isInParty())
			return 0.;
		for (L1PcInstance member : player.getParty().getMembers()) {
			if (member.isCrown() && (player.knownsObject(member) || player.equals(member)))
	...
	private static void AddExp(L1PcInstance pc, int exp, int lawful, int karma) {
		...
		boolean aligned = Integer.signum(karma) == Integer.signum(pc.getKarma());
