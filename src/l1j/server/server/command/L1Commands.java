...
public class L1Commands {
	...
	private static L1Command fromResultSet(ResultSet rs) throws SQLException {
		...
		return new L1Command(rs.getString("name"), rs.getInt("access_level"), rs.getString("class_name"), helpText,
				runOnLogin);
	}
	...
	public static List<L1Command> availableCommandList(int accessLevel) {
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement("SELECT * FROM commands WHERE access_level <= ? ORDER BY name");
