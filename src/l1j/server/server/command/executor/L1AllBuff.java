...
public class L1AllBuff implements L1CommandExecutor {
	...
	public void execute(L1PcInstance pc, String cmdName, String arg) {
		int[] allBuffSkill = { LIGHT, DECREASE_WEIGHT, PHYSICAL_ENCHANT_DEX, MEDITATION, PHYSICAL_ENCHANT_STR,
				BLESS_WEAPON, BERSERKERS, IMMUNE_TO_HARM, ADVANCE_SPIRIT, REDUCTION_ARMOR, BOUNCE_ATTACK,
				SOLID_CARRIAGE, ENCHANT_VENOM, BURNING_SPIRIT, VENOM_RESIST, DOUBLE_BRAKE, UNCANNY_DODGE, DRESS_EVASION,
				RESIST_MAGIC, CLEAR_MIND, ELEMENTAL_PROTECTION, AQUA_PROTECTER, BURNING_WEAPON, IRON_SKIN,
		...
		try {
			...
			for (int element : allBuffSkill) {
				L1Skill skill = SkillTable.getInstance().findBySkillId(element);
				new L1SkillUse().handleCommands(target, element, target.getId(), target.getX(), target.getY(),
						null, skill.getBuffDuration() * 1000, L1SkillUse.TYPE_GMBUFF, null, true);
			}
		} catch (Exception e) {
