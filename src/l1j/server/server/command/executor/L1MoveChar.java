...
public class L1MoveChar implements L1CommandExecutor {
	...
	private TreeMap<String, int[]> _moveLocations = new TreeMap<>(String.CASE_INSENSITIVE_ORDER);
		...
		try {
			...
			if (args.size() == 4) {
				...
				if (_moveLocations.containsKey(inputName)) {
					...
					if (possibleLocations.isEmpty())
						possibleLocations.add(String.format("Locations found matching \"%s\". "
								+ "Try searching on just the first few characters or wrapping multi-word locations in quotes.",
								inputName));

					pc.sendPackets(new S_RawStringDialog(pc.getId(), String.format("Offline Move Locations"),
			...
			if (target != null) {
				...
				pc.sendPackets(new S_SystemMessage(
						String.format("Character '%s' Moved To: %s, %d, %d.", target.getName(), locx, locy, mapid)));
				...
				if (target == null) {
					pc.sendPackets(
							new S_SystemMessage("A character with the name '" + characterName + "' does not exist."));
					return;
				}
