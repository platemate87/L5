...
public class L1LazyGm implements L1CommandExecutor {
	...
	private static HashMap<Integer, LazyGmTimer> _lazyGmTimers = new HashMap<>();
		...
		public void run() {
			try {
				...
				while (!_stop && _lazyGmTimers.size() > 0) {
					try {
						ArrayList<L1PcInstance> onlinePlayers = new ArrayList<>();
						...
						if (onlinePlayers.size() < 3) {
							_gm.sendPackets(
									new S_SystemMessage("Not enough players online to continue. Stopping lazygm."));
							break;
						}
						...
						// limit of 10 just in case something funky goes wrong and it can never find
						// someone
	...
	public void execute(L1PcInstance pc, String cmdName, String arg) {
		try {
			...
			pc.sendPackets(
					new S_SystemMessage("LazyGm has been started. You will move every " + seconds + " seconds."));
