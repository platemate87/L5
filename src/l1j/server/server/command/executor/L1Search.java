...
public class L1Search implements L1CommandExecutor {
	...
	public void execute(L1PcInstance pc, String cmdName, String arg) {
		try {
			...
			while (st.hasMoreTokens()) {
				...
				if (itCount == 0 && (tempVar.equals("armor") || tempVar.equals("weapon") || tempVar.equals("misci")
						|| tempVar.equals("npc") || tempVar.equals("map"))) {
					simpleS = false;
					type = tempVar;
				} else {
		...
		try {
			...
			if (type.equals("misci")) {
				...
				statement = con.prepareStatement(
						"select mapid,locationname,startx,endx,starty,endy from mapids where locationname Like concat('%',?,'%')");
			} else {
				error = true;
	...
	private void find_object(L1PcInstance pc, String name) {
		...
		try {
			...
			statement = con.prepareStatement("select gfx_id,name from polymorphs where name Like concat('%',?,'%')");
			statement.setString(1, name);
			int polyCount = 0;
			rs = statement.executeQuery();
			while (rs.next()) {
			...
			while (rs.next()) {
				...
				pc.sendPackets(new S_SystemMessage(
						String.format("mapid: %s name: %s, startx: %s endx: %s, starty: %s endy: %s", rs.getString(1),
			...
			if (mapCount > 0)
				found += mapCount + " maps.";
			found = found.length() > 0 ? found.substring(0, found.length() - 1) + "." : " Nothing";
			pc.sendPackets(new S_SystemMessage(found));
		} catch (Exception e) {
			_log.error("", e);
		} finally {
