...
public class L1Kill implements L1CommandExecutor {
	...
	public void execute(L1PcInstance pc, String cmdName, String arg) {
		try {
				...
				for (L1PcInstance player : players) {
					...
					if (player.getAccessLevel().getLevel() >= killAccessLevel)
						player.sendPackets(new S_SystemMessage(
								"GM .kill broadcast " + (broadcast ? "enabled" : "disabled") + "."));
				} // end for
			...
			if (target != null && target.getAccessLevel().getLevel() < pc.getAccessLevel().getLevel()) {
				...
				if (broadcast) {
					...
					world.broadcastServerMessage(String.format("A GM has just slaughtered %s.", target.getName()));
				}
			...
			pc.sendPackets(new S_SystemMessage(String.format(".%1$s <player_name> or .%1$s -broadcast", cmdName)));
		}
	}
}
