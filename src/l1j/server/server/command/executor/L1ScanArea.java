	...
	public void execute(L1PcInstance pc, String cmdName, String scanType) {
		...
		try {
			if (scanType.equals("-mob")) {
				HashMap<Integer, ArrayList<String>> visibleMobs = new HashMap<>();
				...
				for (L1Object visibleObject : L1World.getInstance().getVisibleObjects(pc)) {
					if (visibleObject instanceof L1MonsterInstance) {
						...
						if (!visibleMobs.containsKey(mobInstance.getNpcTemplate().get_npcId())) {
							visibleMobs.put(mobInstance.getNpcTemplate().get_npcId(), new ArrayList<>(
									Arrays.asList(mobInstance.getName(), "" + mobInstance.getLevel())));
						}
					}
				}
				...
				List<Entry<Integer, ArrayList<String>>> list = new ArrayList<>(set);
				...
				for (Map.Entry<Integer, ArrayList<String>> entry : list) {
					message.append(String.format("%s\nLevel: %s, Template Id: %s\n\n", entry.getValue().get(0),
							entry.getValue().get(1), entry.getKey()));

				}

				pc.sendPackets(new S_CustomBoardRead("Scan Results", pc.getName(), message.toString()));
