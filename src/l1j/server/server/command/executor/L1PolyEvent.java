...
public class L1PolyEvent implements L1CommandExecutor {
	...
	public void execute(L1PcInstance pc, String cmdName, String arg) {
		...
		String currentStatusMessage = "Current Status is: " + (currentlyEnabled ? "Enabled" : "Disabled");
		...
		try {
			...
			if ((enabled && currentlyEnabled) || (!enabled && !currentlyEnabled)) {
				pc.sendPackets(
						new S_SystemMessage("\\fRPolymorph Event Already " + (enabled ? "Enabled" : "Disabled") + "!"));
			...
			pc.sendPackets(new S_SystemMessage("\\fRPolymorph Event " + (enabled ? "Enabled" : "Disabled") + "!"));
