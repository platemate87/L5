		...
		try {
			...
			if (tg != null) {
				if (time == 0) {
					...
					pc.sendPackets(new S_SystemMessage(String.format("You have given %s back their voice.", name)));
				} else {
					tg.setSkillEffect(STATUS_CHAT_PROHIBITED, time * 60 * 1000);
					tg.sendPackets(new S_ServerMessage(286, String.valueOf(time)));

					pc.sendPackets(new S_SystemMessage(
							String.format("You have prohibited %s from chatting for %d minutes.", name, time)));
				}
			}
		} catch (Exception e) {
