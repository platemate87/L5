...
public class L1Status implements L1CommandExecutor {
	...
	public void execute(L1PcInstance pc, String cmdName, String arg) {
		try {
			...
			pc.sendPackets(new S_SystemMessage("Changed " + target.getName() + "'s " + param + " to " + value));
		} catch (Exception e) {
			pc.sendPackets(new S_SystemMessage(
					String.format(".%1$s <player> <setting> <value> or .%1$s <setting> <value>", cmdName)));
			pc.sendPackets(new S_RawStringDialog(pc.getId(), "Available settings:",
					"AC, MR, HIT, DMG, HP, MP, LAWFUL, KARMA, STR, CON, DEX, INT, WIS, CHA, PINK"));
		}
	}
}
