...
public class L1Promote implements L1CommandExecutor {
	...
	public void execute(L1PcInstance pc, String cmdName, String arg) {
		try {
			...
			ArrayList<L1AccessLevel> availableLevels = new ArrayList<>();
			for (L1AccessLevel level : AccessLevelTable.getInstance().getAllAccessLevels()) {
				if (level.getLevel() < pc.getAccessLevel().getLevel() || pc.getAccessLevel().getLevel() == FULL_GM)
			...
			if (playerName.equals("-list")) {
				...
				for (L1AccessLevel level : availableLevels)
					message.append(
							String.format("ID#: %d, %s (%d)\n", level.getId(), level.getName(), level.getLevel()));

				pc.sendPackets(new S_CustomBoardRead("Access Levels", pc.getName(), message.toString()));
			...
			String promoteDemote = target.getAccessLevel().getLevel() > levelToChangeTo.getLevel() ? "demoted"
					: "promoted";

			target.setAccessLevel(levelToChangeTo);
			target.sendPackets(new S_SystemMessage(
					String.format("You have been %s to %s.", promoteDemote, levelToChangeTo.getName())));

			pc.sendPackets(new S_SystemMessage(String.format("%s has been %s to %s.", target.getName(), promoteDemote,
					levelToChangeTo.getName())));
