...
public class L1Jail implements L1CommandExecutor {
		...
		try {
			...
			if (convict != null) {
				...
				convict.sendPackets(new S_SystemMessage(
						String.format("%s jailed you for %d %s.", pc.getName(), duration, durationType)));
			...
			if (broadcast) {
				...
				world.broadcastServerMessage(String.format("%s has been jailed for %d %s for bad behaviour!",
						convict.getName(), duration, durationType));
			}

			pc.sendPackets(new S_SystemMessage(
					String.format("%s has been jailed for %d %s.", convict.getName(), duration, durationType)));
	...
	private static int getExistingJailId(String user) {
		...
		try {
			...
			pstm = con
					.prepareStatement("SELECT * FROM `log_jail` WHERE `status` = 'jailed' AND `jailed_char_name` = ?");
			...
			_log.warn("Failed to look up existing user ban. Could lead to duplicate entries in the log_jail table!",
					ex);
		} finally {
			SQLUtil.close(rs);
			SQLUtil.close(pstm);
		...
		try {
			...
			if (existingJailId == -1) {
				...
				pstm = con.prepareStatement(
						"UPDATE `log_jail` SET `jailer` = ?, message = ?, unjail = ?, modified = NOW()");
