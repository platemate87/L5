			...
			if (target == null) {
				...
				try {
					try {
						...
						L1PolyMorph.doPoly(target, polyId, 7200, L1PolyMorph.MORPH_BY_GM);
					} catch (NumberFormatException nfe) {
						// if it fails, it wasn't a number, so lets assume it is a name
						// look up the name and see if we can morph them
						TreeMap<String, L1PolyMorph> polymorphs = new TreeMap<>(
						...
						if (polymorphs.containsKey(polyIdOrName)) {
							...
							if (possibleMorphs.isEmpty())
								possibleMorphs.add(String.format("No morphs found matching \"%s\". "
										+ "Try searching on just the first few characters or wrapping multi-word morphs in quotes.",
										polyIdOrName));

							pc.sendPackets(new S_RawStringDialog(pc.getId(), "Polymorph not found. Did you mean?",
									Joiner.on(", ").join(possibleMorphs)));
						}
					}
				} catch (Exception exception) {
					pc.sendPackets(new S_SystemMessage(String
							.format(".%1$s <player> <poldId or \"name\"> or .%1$s <poldId or \"name\">", cmdName)));
				}
			}
		} catch (Exception e) {
