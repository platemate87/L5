...
public class L1Resolve implements L1CommandExecutor {
	...
	public void execute(L1PcInstance pc, String cmdName, String arg) {
		...
		try {
			...
			pstm.execute();
			pc.sendPackets(new S_SystemMessage("Bug #" + bugNumber + " has been resolved!"));
			pstm.close();
			...
			byte[] mailText = "B\0u\0g\0 \0R\0e\0s\0o\0l\0v\0e\0d\0!\0\0\0Y\0o\0u\0r\0 \0b\0u\0g\0 \0h\0a\0s\0 \0b\0e\0e\0n\0 \0r\0e\0s\0o\0l\0v\0e\0d\0!\0\0\0\0"
					.getBytes("UTF-8");
			...
			if (receiver != null) {
				MailTable.getInstance().writeMail(0, submitterName, receiver, mailText, receiver.getId());

				if (receiver.getOnlineStatus() == 1) {
					receiver.sendPackets(new S_Mail(receiver, 0));
					receiver.sendPackets(new S_SystemMessage(
							"A bug you submitted has been resolved. " + "Check your mail for more info!"));
				}
			} else {
				try {
					L1PcInstance restorePc = CharacterTable.getInstance().restoreCharacter(submitterName);

					if (restorePc != null) {
						MailTable.getInstance().writeMail(0, submitterName, restorePc, mailText, restorePc.getId());
					} else {
