		...
		try {
			...
			if (args.length > 1) {
				...
				try {
					...
					pc.sendPackets(
							new S_SystemMessage("Use the .scan command to find the template ID of mobs around you!"));
					return;
				}
				...
				if (npcTemplate == null) {
					pc.sendPackets(new S_SystemMessage(String.format("Unable to find a mob with id %d!", mobId)));
					return;
				}
				...
				pc.sendPackets(new S_SystemMessage(
						String.format("Now spoofing %s (LvL %d)!", spoofMob.getName(), spoofMob.getLevel())));
			} else {
				if (arg.contains("-")) {
					throw new Exception();
				}
				...
				if (playerToSpoof == null
					...
					pc.sendPackets(
							new S_SystemMessage("You cannot spoof someone with a higher access level than yourself!"));
				}
			}
		} catch (Exception e) {
