...
public class CheckIP implements L1CommandExecutor {
	...
	private void ipCheck(L1PcInstance gm, String name) throws Exception {
		...
		if (target == null) {
			gm.sendPackets(new S_SystemMessage(".ipcheck doesn't work on offline players."));
			return;
		} else {
			gm.sendPackets(new S_SystemMessage((new StringBuilder()).append(target.getName() + ": ")
					.append("IP: " + target.getNetConnection().getIp()).toString()));
		}
	}
}
