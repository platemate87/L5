...
public class L1ViewBug implements L1CommandExecutor {
	...
	public void execute(L1PcInstance pc, String cmdName, String arg) {
		...
		try {
			...
			if (rs.getInt("resolved") == 1) {
				pc.sendPackets(new S_SystemMessage(rs.getInt("id") + "(Resolved). " + rs.getString("charname")
						+ " (on Map:" + rs.getInt("mapID") + ", X:" + rs.getInt("mapX") + ", Y:" + rs.getInt("mapY")
						+ ") wrote: " + rs.getString("bugtext") + "."));
			} else {
				pc.sendPackets(new S_SystemMessage(rs.getInt("id") + ". " + rs.getString("charname") + " (on Map:"
						+ rs.getInt("mapID") + ", X:" + rs.getInt("mapX") + ", Y:" + rs.getInt("mapY") + ") wrote: "
	...
	private void listBugs(L1PcInstance pc) {
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement("select * from bugs where resolved = 0");// ORDER
																					// BY
																					// id
			rs = pstm.executeQuery();
			while (rs.next()) {
				...
				pc.sendPackets(
						new S_SystemMessage(rs.getInt("id") + ": " + rs.getString("bugtext").substring(0, str1length)));
			}
		} catch (Exception e) {
