...
public class L1Cancel implements L1CommandExecutor {
	...
	public void execute(L1PcInstance pc, String cmdName, String char_name) {
		try {
			...
			if (target == null) {
				pc.sendPackets(new S_SystemMessage(String.format("Player '%1$s' is not online!", char_name)));
				return;
			}

			this.cancelPlayer(target);

			pc.sendPackets(new S_SystemMessage(String.format("Player '%1$s' has been cancelled!", char_name)));
		} catch (Exception e) {
			pc.sendPackets(new S_SystemMessage(String.format(".%1$s <player_name>", cmdName)));
		}
	}
	...
	private void cancelPlayer(L1PcInstance player) throws Exception {
		...
		for (int skillNum = SKILLS_BEGIN; skillNum <= SKILLS_END; skillNum++) {
			// if it isn't a cancellable skill, we need to send the packet manually to the
			// client
