...
public class L1Move implements L1CommandExecutor {
	...
	private TreeMap<String, TreeMap<String, int[]>> _teleportLocations = new TreeMap<>(
			String.CASE_INSENSITIVE_ORDER);

	private L1Move() {
		TreeMap<String, int[]> _towns = new TreeMap<>(String.CASE_INSENSITIVE_ORDER);
		TreeMap<String, int[]> _castles = new TreeMap<>(String.CASE_INSENSITIVE_ORDER);
	...
	public void execute(L1PcInstance pc, String cmdName, String arg) {
		try {
			...
			if (args.size() == 3) {
				...
				if (_teleportLocations.get(type).containsKey(inputName)) {
					...
					if (possibleLocations.isEmpty())
						possibleLocations.add(String.format("Locations (%s) found matching \"%s\". "
								+ "Try searching on just the first few characters or wrapping multi-word locations in quotes.",
								type, inputName));

					pc.sendPackets(new S_RawStringDialog(pc.getId(), String.format("Teleport Locations (%s)", type),
			...
			if (locx != pc.getX() || locy != pc.getY() || mapid != pc.getMapId()) {
				...
				pc.sendPackets(new S_SystemMessage("Moved: " + locx + ", " + locy + ", " + mapid + ""));
			}
		} catch (Exception e) {
