...
public class BuffCheck implements L1CommandExecutor {
	...
	private void buffCheck(L1PcInstance gm, String name) throws Exception {
		...
		if (target == null) {
			gm.sendPackets(new S_SystemMessage(".buffCheck doesn't work on offline players yet."));
			return;
		} else {
			...
			for (Map.Entry<Integer, L1SkillTimer> buff : buffs.entrySet()) {
				String time = buff.getValue() != null ? ((Integer) buff.getValue().getRemainingTime()).toString()
						: "forever";
				buffList.append("| " + L1NamedSkill.getName(buff.getKey()) + " " + time);
			}
			gm.sendPackets(new S_SystemMessage(name + " currently has: " + buffList.toString()));
		}
	}
}
