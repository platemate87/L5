...
public class L1ToggleShop implements L1CommandExecutor {
	...
	private static HashMap<Integer, ShopStatus> disabledNpcs = new HashMap<>();
	private static boolean allBuyDisabled = false;
	private static boolean allSellDisabled = false;

	private static String[] commandMessage = { ".toggleshop <npcid> <buy/sell> <on/off> or ",
			".toggleshop <buy/sell> <on/off> or ", ".toggleshop status" };
	...
	public void execute(L1PcInstance pc, String cmdName, String arg) {
		try {
				...
				if (!allBuyDisabled && !allSellDisabled) {
					if (disabledNpcs.isEmpty()) {
						...
						while (shops.hasNext()) {
							...
							pc.sendPackets(new S_SystemMessage(String.format("%s: %s. %s.", npc.get_name(),
			...
			if (!npcName.equals("")) {
				...
				pc.sendPackets(new S_SystemMessage(String.format("NPC '%s' %s shop %s.", npc.get_name(), buySell,
				...
				// the npcs we set intially, and clear them out if they are no longer disabled
				// at all
				Iterator<Entry<Integer, ShopStatus>> shops = disabledNpcs.entrySet().iterator();
				while (shops.hasNext()) {
					Entry<Integer, ShopStatus> shop = shops.next();
				...
				pc.sendPackets(new S_SystemMessage(
						String.format("All %s shops have been %s.", buySell, disabled ? "disabled" : "enabled")));
			} // end if
		} catch (Exception e) {
			for (String element : commandMessage)
				pc.sendPackets(new S_SystemMessage(element));
		}
	}
}
