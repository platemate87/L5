	...
	public void execute(L1PcInstance pc, String cmdName, String arg) {
		try {
			...
			if (target == null && arg.toLowerCase().equals("-rand")) {
				ArrayList<L1PcInstance> players = new ArrayList<>(L1World.getInstance().getAllPlayers());
			...
			if (target != null) {
				...
				pc.sendPackets(new S_SystemMessage(
						(new StringBuilder()).append("You appear next to ").append(target.getName()).toString() + "."));
			} else {
				pc.sendPackets(
						new S_SystemMessage((new StringBuilder()).append(arg).append(" is not online.").toString()));
			}
		} catch (Exception e) {
