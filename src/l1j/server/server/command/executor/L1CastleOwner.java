...
public class L1CastleOwner implements L1CommandExecutor {
			...
			if (args.length == 1) {
				...
				if (castleOwnerClan == null) {
					...
					pc.sendPackets(new S_SystemMessage(
							String.format("%s's owner is %s.", castle.getName(), castleOwnerClan.getClanName())));
				}
				...
				if (castleOwnerClanName == null) {
					...
					pc.sendPackets(new S_SystemMessage(
							String.format("%s is now the owner of %s!", newOwnerClan.getClanName(), castle.getName())));
					return;
				} else {
					...
					if (currentOwnerChar != null) {
						...
						// remove the crown from the player -- need to send to the player that someone
						// else owns the castle
						...
						currentOwnerChar.sendPackets(new S_SystemMessage(
								String.format("%s has been taken from you by a GM!", castle.getName())));
					}
					...
					if (newOwnerClanName == null) {
						if (!pledgeName.equals("-none")) {
							pc.sendPackets(new S_SystemMessage(String.format(
									"No clan with name %s found. %s is now vacant.", pledgeName, castle.getName())));
						} else {
	...
	private String getCastleOwner(int castleId) {
		...
		try {
			...
			pstm = con.prepareStatement("SELECT clan_name FROM clan_data WHERE hasCastle = ?");
