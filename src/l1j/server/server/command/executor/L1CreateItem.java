		...
		try {
			...
			if (args.size() == 4) {
				try {
					...
					int itemId = ItemTable.getInstance().findItemIdByNameWithoutSpace(nameOrId, true);
			...
			if (itemToCreate != null) {
				...
				List<L1ShopItem> shopItems = new ArrayList<>();
				...
				pc.sendPackets(new S_SystemMessage(String.format(
						".%1$s <itemid or name> <amount> <enchant> <isID> or .%1$s <name> <enchant>", cmdName)));
			}
		} catch (Exception e) {
			_log.error(e.getLocalizedMessage(), e);
			pc.sendPackets(new S_SystemMessage(String
					.format(".%1$s <itemid or name> <amount> <enchant> <isID> or .%1$s <name> <enchant>", cmdName)));
		}
	}
}
