				...
				if (action.equals("ban")) {
					...
					for (String char_name : playerAccount.getCharacters()) {
						...
						if (character != null) {
							...
							pc.sendPackets(
									new S_SystemMessage(String.format("Character '%s' in account '%s' has been kicked!",
					...
					pc.sendPackets(new S_SystemMessage(
							String.format("The account '%s' has been banned.", playerAccount.getName())));
				} else if (action.equals("unban")) {
					if (!playerAccount.isBanned()) {
						pc.sendPackets(new S_SystemMessage(String.format("Account '%s' is not banned!", accountName)));
						return;
					}

					Account.unban(playerAccount.getName(), pc.getAccountName(), message.toString());
					pc.sendPackets(new S_SystemMessage(
							String.format("The account '%s' has been unbanned.", playerAccount.getName())));
					...
					for (BanInfo historyItem : banHistory) {
						String accountInfoMessage = String.format("%s: \n\n%s by %s\nReason:\n%s\n\n",
								formatter.format(historyItem.getActionDate()), historyItem.getAction(),
								historyItem.getActionerAccountName(), historyItem.getMessage());
						banHistoryString.append(accountInfoMessage);
					}

					pc.sendPackets(new S_CustomBoardRead(playerAccount.getName() + " Ban History", pc.getName(),
							banHistoryString.toString()));
				} else {
					throw new Exception();
				}
			} else {
