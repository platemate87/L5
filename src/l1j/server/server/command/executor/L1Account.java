...
public class L1Account implements L1CommandExecutor {
	...
	public void execute(L1PcInstance pc, String cmdName, String arg) {
		...
		try {
			...
			pstm = con.prepareStatement(
					"SELECT char_name,level,MaxHp,MaxMp,Class,objid,account_name,accounts.banned FROM characters"
			...
			pstm3 = con.prepareStatement(
					"SELECT count FROM character_elf_warehouse WHERE account_name=? AND item_id = 40308");
			...
			while (rs.next()) {
				...
				message.append(rs.getString("char_name")).append(": Level ")
						.append(rs.getInt("level") + (online ? " (Online)" : "")).append("\n")
						.append(getSex(rs.getInt("Class"))).append(" ").append(getClass(rs.getInt("Class")))
			...
			if (!actualAccountName.equals("")) {
				...
				pc.sendPackets(
						new S_CustomBoardRead("Account: " + actualAccountName, pc.getName(), message.toString()));
			} else {
	...
	private String getSex(int classID) {
		if (classID == 0 || classID == 61 || classID == 138 || classID == 734 || classID == 2786 || classID == 6658
				|| classID == 6671) {
			return "Male";
		} else if (classID == 1 || classID == 48 || classID == 37 || classID == 1186 || classID == 2796
				|| classID == 6661 || classID == 6650) {
			return "Female";
		} else {
			return "error";
		}
