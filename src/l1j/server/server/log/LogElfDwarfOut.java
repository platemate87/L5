...
public class LogElfDwarfOut {
	private static Logger _log = LoggerFactory.getLogger(LogElfDwarfOut.class.getName());
	private static final SimpleDateFormat FORMATTER = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");

	public void storeLogElfDwarfOut(L1PcInstance pc, L1ItemInstance item, int item_count_before, int item_count_after,
			int item_out_count) {
		Connection con = null;
		...
		try {
			...
			pstm = con.prepareStatement(
					"INSERT INTO logelfwarehouseout VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);");
			Date time = new Date();
