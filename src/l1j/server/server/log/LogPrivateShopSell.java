...
public class LogPrivateShopSell {
	private static Logger _log = LoggerFactory.getLogger(LogPrivateShopSell.class.getName());

	public long storeLogPrivateShopSell(String transactionId, L1PcInstance pc, L1PcInstance target, L1ItemInstance item,
			int itembefore, int sellcount) {
		Connection con = null;
		...
		try {
			...
			pstm = con.prepareStatement(
					"INSERT INTO logprivateshopsell (Time, Ip, Account, CharId, CharName, TargetIp, TargetAccount, TargetCharId, "
							+ ""
		...
		if (generatedKey == -1) {
			_log.warn(String.format(
					"Failed to insert log for private shop sell! TransactionId: %s, Seller: %s, Shop: %s, Item: %s, ItemId: %d, "
	...
	public void completeTransaction(long id, int item_before_count, int item_after_count) {
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement(
					"UPDATE LogPrivateShopSell SET `Completed` = '1', `ItemAfter` = ?, `ItemDiff` = ? WHERE Id = ?");
