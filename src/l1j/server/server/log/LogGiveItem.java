...
public class LogGiveItem {
	...
	public void storeLogGiveItem(L1PcInstance pc, L1Character target, L1ItemInstance item, int before_inven,
			int after_inven, int before_target_inv, int after_target_inv, int count) {
		Connection con = null;
		PreparedStatement pstm = null;

		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement("INSERT INTO loggiveitem VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?);");
			Date time = new Date();
