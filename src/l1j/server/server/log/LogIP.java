...
public class LogIP {
	...
	public void storeLogIP(L1PcInstance pc, String ip) {
		...
		try {
			...
			pstm = con.prepareStatement(
					"INSERT INTO logip (LoginTime, Ip, Account, CharName, LogoutTime)" + " VALUES (?, ?, ?, ?, ?);");
			Date time = new Date();
	...
	public void storeLogout(L1PcInstance pc) {
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement("UPDATE logip set logouttime=? where logouttime='null' and charname=?;");
			Date time = new Date();
			SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
			String fm = formatter.format(time.getTime());
			pstm.setString(1, fm);
