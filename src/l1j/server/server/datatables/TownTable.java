...
public class TownTable {
	...
	public synchronized void addSalesMoney(int town_id, L1ShopBuyOrderList orderList) {
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement("UPDATE town SET sales_money = sales_money + ?, "
	...
	public void removeTaxes(int town_id) {
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement("UPDATE town SET town_tax = 0 WHERE town_id = ?");
	...
	public void changeTaxRate(int town_id, int tax_rate) {
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement("UPDATE town SET tax_rate_reserved = ? WHERE town_id = ?");
