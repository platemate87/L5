...
public class DoorTable {
	...
	private void loadDoors() {
		for (L1DoorSpawn spawn : L1DoorSpawn.all()) {
			...
			if (_doors.containsKey(loc)) {
				_log.warn(String.format("Duplicate door location: id = %d", spawn.getId()));
				continue;
			}
			createDoor(spawn.getId(), spawn.getGfx(), loc, spawn.getHp(), spawn.getKeeper());
		}
	}
	...
	public L1DoorInstance createDoor(int doorId, L1DoorGfx gfx, L1Location loc, int hp, int keeper) {
		if (_doors.containsKey(loc)) {
			return null;
		}
	...
	public L1DoorInstance findByDoorLoc(int x, int y, int map) {
		for (L1DoorInstance door : _doors.values()) {
			if (door.getLocation().getX() == x && door.getLocation().getY() == y
					&& door.getLocation().getMapId() == map) {
				return door;
			}
		}
		return null;
