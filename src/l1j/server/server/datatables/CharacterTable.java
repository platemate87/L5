...
public class CharacterTable {
	...
	private static Logger _log = LoggerFactory.getLogger(CharacterTable.class.getName());
	...
	public static void saveCharStatus(L1PcInstance pc) {
		...
		try {
			...
			pstm = con.prepareStatement(
					"UPDATE characters SET OriginalStr= ?" + ", OriginalCon= ?, OriginalDex= ?, OriginalCha= ?"
	...
	public Collection<L1PcInstance> getOfflineInZone(int castleId) {
		...
		PreparedStatement pstm = null;
		ResultSet rs = null;
		Collection<L1PcInstance> offlineInZone = new ArrayList<>();
		...
		try {
			...
			while (rs.next()) {
				try {
					...
					_log.warn("Unable to get offline characters name: " + e.getLocalizedMessage(), e);
				}
			}
		} catch (SQLException e) {
			_log.error(e.getLocalizedMessage(), e);
