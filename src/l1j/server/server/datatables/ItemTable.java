...
public class ItemTable {
	...
	private Map<Integer, L1EtcItem> allEtcItem() {
		...
		try {
			...
			_log.error(new StringBuilder().append(item.getName()).append("(" + item.getItemId() + ")")
	...
	private Map<Integer, L1Weapon> allWeapon() {
		...
		try {
			...
			_log.error(
					...
					new StringBuilder().append(weapon.getName()).append("(" + weapon.getItemId() + ")")
							.append(" Failed to load.").toString());
		} catch (SQLException e) {
			_log.error(e.getLocalizedMessage(), e);
		} finally {
	...
	private Map<Integer, L1Armor> allArmor() {
		...
		try {
			...
			_log.error(
					...
					new StringBuilder().append(armor.getName()).append("(" + armor.getItemId() + ")")
							.append(" Failed to load.").toString());
		} catch (SQLException e) {
			_log.error(e.getLocalizedMessage(), e);
		} finally {
	...
	public List<L1Item> findItemsByName(String name, boolean fuzzy) {
		List<L1Item> itemsFound = new ArrayList<>();
		String lcaseName = name.toLowerCase();

		for (L1Item item : _allTemplates) {
			if (item != null
					&& (item.getName().equals(name) || (fuzzy && item.getName().toLowerCase().contains(lcaseName)))) {
				itemsFound.add(item);
			}
		}
