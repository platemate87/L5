...
public class AuctionBoardTable {
	private static Logger _log = LoggerFactory.getLogger(AuctionBoardTable.class.getName());
//	private static AuctionBoardTable _instance;
	...
	public void insertAuctionBoard(L1AuctionBoard board) {
		...
		try {
			...
			pstm = con.prepareStatement(
					"INSERT INTO board_auction SET house_id=?, house_name=?, house_area=?, deadline=?, price=?, location=?, old_owner=?, old_owner_id=?, bidder=?, bidder_id=?");
	...
	public void updateAuctionBoard(L1AuctionBoard board) {
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement(
					"UPDATE board_auction SET house_name=?, house_area=?, deadline=?, price=?, location=?, old_owner=?, old_owner_id=?, bidder=?, bidder_id=? WHERE house_id=?");
