...
public class ExcludeTable {
	...
	private ExcludeTable() {
		...
		try {
			...
			charIdPS = con.prepareStatement("SELECT distinct(char_id) as char_id FROM character_excludes");
			charIdRS = charIdPS.executeQuery();
			while (charIdRS.next()) {
				PreparedStatement excludesPS = null;
				ResultSet excludesRS = null;
				try {
					excludesPS = con.prepareStatement(
							"SELECT exclude_id, exclude_name FROM character_excludes WHERE char_id = ?");
					...
					while (excludesRS.next()) {
						exclude.add(excludesRS.getInt("exclude_id"), excludesRS.getString("exclude_name"));
					}
	...
	public void addExclude(int charId, int objId, String name) {
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement("INSERT INTO character_excludes SET char_id=?, exclude_id=?, exclude_name=?");
	...
	public void removeExclude(int charId, String excludeName) {
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement("DELETE FROM character_excludes WHERE char_id=? AND exclude_name=?");
			pstm.setInt(1, charId);
